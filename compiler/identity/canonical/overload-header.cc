...
#include "compiler/identity/canonical/overload-header-data.h"
...
OverloadHeader::OverloadHeader(
    zc::Own<detail::CanonicalOverloadHeaderData>&& value) noexcept
    : impl(zc::mv(value)) {}
OverloadHeader::~OverloadHeader() noexcept(false) = default;
OverloadHeader::OverloadHeader(OverloadHeader&&) noexcept = default;
OverloadHeader& OverloadHeader::operator=(OverloadHeader&&) noexcept =
    default;

zc::Maybe<OverloadHeader> OverloadHeader::from(
...
OverloadHeader OverloadHeader::clone() const {
...
CallableHeaderKind OverloadHeader::callableKind() const noexcept {
  return impl->callableKind;
}

zc::StringPtr OverloadHeader::name() const noexcept { return impl->name.text(); }

zc::Maybe<ReceiverShape> OverloadHeader::receiver() const noexcept {
  ZC_IF_SOME(value, impl->receiver) { return value; }
  return zc::none;
}

zc::ArrayPtr<const CanonicalGenericParameter> OverloadHeader::genericParameters()
    const noexcept {
  return impl->genericParameters.asPtr();
}

zc::ArrayPtr<const CanonicalBoundObligation> OverloadHeader::obligations() const noexcept {
  return impl->obligations.asPtr();
}

zc::ArrayPtr<const CanonicalCallableParameter> OverloadHeader::parameters()
    const noexcept {
  return impl->parameters.asPtr();
}

const CanonicalCallableResult& OverloadHeader::result() const noexcept {
  return impl->result;
}

zc::Maybe<zc::ArrayPtr<const CanonicalHeaderTypeSyntax>> OverloadHeader::raises()
    const noexcept {
  ZC_IF_SOME(values, impl->raises) { return values.asPtr(); }
  return zc::none;
}

zc::Maybe<ExternalAbi> OverloadHeader::externalAbi() const noexcept {
  ZC_IF_SOME(value, impl->externalAbi) { return value; }
  return zc::none;
}

void OverloadHeader::encode(CanonicalEncoder& encoder) const {
...
zc::Array<uint8_t> OverloadHeader::encode() const {
  CanonicalEncoder encoder;
  encode(encoder);
  return encoder.finish();
}
