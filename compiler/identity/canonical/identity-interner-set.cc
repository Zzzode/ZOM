...
struct IdentityInternerSet::Impl final {
...
IdentityInternerSet::~IdentityInternerSet() noexcept(false) = default;
IdentityInternerSet::IdentityInternerSet(
    IdentityInternerSet&&) noexcept = default;
IdentityInternerSet& IdentityInternerSet::operator=(
    IdentityInternerSet&&) noexcept = default;
IdentityInternerSet::IdentityInternerSet(zc::Own<Impl>&& impl) noexcept
    : impl(zc::mv(impl)) {}

zc::Maybe<IdentityInternerSet> IdentityInternerSet::create(
  ...
  return IdentityInternerSet(zc::mv(candidate));
}

SemanticContextBrand IdentityInternerSet::context() const noexcept {
  return impl->context;
}

IdentityInternResult<CompilationUnitId> IdentityInternerSet::internCompilationUnit(
    SemanticContextBrand context, const CompilationUnitIdentity& key) {
  return impl->compilationUnits.intern(context, key, key, true);
}

IdentityInternResult<CrateId> IdentityInternerSet::internCrate(
    SemanticContextBrand context, const CrateKey& key) {
  return impl->crates.intern(context, key, key, true);
}

IdentityInternResult<SourceFileId> IdentityInternerSet::internSourceFile(
    SemanticContextBrand context, const SourceFileKey& key) {
  return impl->sourceFiles.intern(context, key, key, true);
}

IdentityInternResult<ModuleId> IdentityInternerSet::internModule(
    SemanticContextBrand context, const ModuleKey& key) {
  return impl->modules.intern(context, key, key, true);
}

IdentityInternResult<DefId> IdentityInternerSet::internDefinition(
...
IdentityInternResult<ImplId> IdentityInternerSet::internImplementation(
...
IdentityInternResult<GenericParameterId> IdentityInternerSet::internGenericParameter(
...
IdentityInternResult<CallableParameterId> IdentityInternerSet::internCallableParameter(
...
zc::Maybe<CompilationUnitIdentityEntry> IdentityInternerSet::compilationUnit(
...
zc::Maybe<CrateIdentityEntry> IdentityInternerSet::crate(CrateId handle) const {
  return impl->crates.lookup(handle);
}

zc::Maybe<CrateIdentityEntry> IdentityInternerSet::crate(const CrateKey& key) const {
  return impl->crates.lookup(key);
}

zc::Maybe<SourceFileIdentityEntry> IdentityInternerSet::sourceFile(
...
zc::Maybe<ModuleIdentityEntry> IdentityInternerSet::module(ModuleId handle) const {
  return impl->modules.lookup(handle);
}

zc::Maybe<ModuleIdentityEntry> IdentityInternerSet::module(const ModuleKey& key) const {
  return impl->modules.lookup(key);
}

zc::Maybe<DefinitionIdentityEntry> IdentityInternerSet::definition(DefId handle) const {
  return impl->definitions.lookup(handle);
}

zc::Maybe<DefinitionIdentityEntry> IdentityInternerSet::definition(
    const DefinitionKey& key) const {
  return impl->definitions.lookup(key);
}

zc::Maybe<ImplementationIdentityEntry> IdentityInternerSet::implementation(
    ImplId handle) const {
...
zc::Maybe<GenericParameterIdentityEntry> IdentityInternerSet::genericParameter(
...
zc::Maybe<CallableParameterIdentityEntry> IdentityInternerSet::callableParameter(
