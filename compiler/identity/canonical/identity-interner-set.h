...
public:
  ~IdentityInternerSet() noexcept(false);
  IdentityInternerSet(IdentityInternerSet&&) noexcept;
  IdentityInternerSet& operator=(IdentityInternerSet&&) noexcept;
  ZC_DISALLOW_COPY(IdentityInternerSet);

  ZC_NODISCARD static zc::Maybe<IdentityInternerSet> create(
...
private:
  struct Impl;
  explicit IdentityInternerSet(zc::Own<Impl>&& impl) noexcept;
  zc::Own<Impl> impl;
};
