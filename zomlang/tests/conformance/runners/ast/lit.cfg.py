...
# -*- coding: utf-8 -*-
"""
Lit configuration for ZOM AST conformance expectations.

The lit tests live under conformance/expectations/ast/*.check. Each CHECK file
contains the RUN line and FileCheck patterns; the corresponding pure ZOM source
lives under conformance/corpus with the same relative path and a .zom suffix.
"""
...
config.name = "ZOM-AST-Conformance"
...
test_root = os.path.abspath(
    os.path.join(conformance_root, "expectations", "ast")
)
...
filecheck_path = os.path.join(
    repo_root, "zomlang", "tests", "tools", "filecheck.py"
)
package_runner_path = os.path.join(
    repo_root, "zomlang", "tests", "tools", "run-zomc-package.py"
)
