...
ImplHeader implHeader(zc::StringPtr traitName) {
  zc::Vector<CanonicalHeaderTypeSyntax> arguments;
  ...
  return require(ImplHeader::from(zc::mv(generics), ImplPolarity::Positive,
...
ZC_TEST("IdentityInternerSet interns and reverses all eight domains") {
  SemanticContextFactory factory;
  const auto owner = context(factory);
  auto interners = require(IdentityInternerSet::create(factory, owner));
...
ZC_TEST("IdentityInternerSet coalesces concurrent equal admission") {
  SemanticContextFactory factory;
  const auto owner = context(factory);
  auto interners = require(IdentityInternerSet::create(factory, owner));
...
ZC_TEST("IdentityInternerSet rejects collisions malformed records and foreign brands") {
  SemanticContextFactory factory;
  const auto owner = context(factory);
  ...
  auto foreignInterners = require(IdentityInternerSet::create(factory, foreign));
...
ZC_TEST("IdentityInternerSet rejects duplicate construction authority") {
  SemanticContextFactory factory;
  const auto owner = context(factory);
  auto interners = require(IdentityInternerSet::create(factory, owner));
  ZC_EXPECT(interners.context() == owner);
  ZC_EXPECT(IdentityInternerSet::create(factory, owner) == zc::none);
}
