...
ImportBindingKey::ImportBindingKey(
...
zc::Maybe<ImportBindingKey> ImportBindingKey::from(
    ModuleKey&& requester, ModuleResolutionKey&& resolution, SemanticImportOperation operation,
    DefinitionNamespace sourceNamespace, DeclaredDefinitionName&& sourceName,
  ...
  return ImportBindingKey(zc::mv(requester), zc::mv(resolution), operation, sourceNamespace,
                                  zc::mv(sourceName), localNamespace, zc::mv(localName));
}

ImportBindingKey ImportBindingKey::clone() const {
  return ImportBindingKey(requesterValue.clone(), resolutionValue.clone(), operationValue,
                                  sourceNamespaceValue, sourceNameValue.clone(),
                                  localNamespaceValue, localNameValue.clone());
}

const ModuleKey& ImportBindingKey::requester() const noexcept { return requesterValue; }

const ModuleResolutionKey& ImportBindingKey::resolution() const noexcept {
  return resolutionValue;
}

SemanticImportOperation ImportBindingKey::operation() const noexcept {
  return operationValue;
}

DefinitionNamespace ImportBindingKey::sourceNamespace() const noexcept {
  return sourceNamespaceValue;
}

const DeclaredDefinitionName& ImportBindingKey::sourceName() const noexcept {
  return sourceNameValue;
}

DefinitionNamespace ImportBindingKey::localNamespace() const noexcept {
  return localNamespaceValue;
}

const DeclaredDefinitionName& ImportBindingKey::localName() const noexcept {
  return localNameValue;
}

zc::Array<uint8_t> ImportBindingKey::encode() const {
...
bool ImportBindingKey::operator==(const ImportBindingKey& other) const {
