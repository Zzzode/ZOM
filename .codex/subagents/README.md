...
| Incremental query runtime, red-green reuse, or projection shielding, including request-result closure | ✅ route → | ↗ if contract changes | ↗ parse provider | ↗ semantic provider | ✅ query database, token identity, evaluator, and descriptor owner | ↗ diagnostic facts | ↗ cancellation interaction | ↗ CMake direction | ✅ architecture drift | ↗ token and memo lifetime | ✅ gates, descriptor generator, real-object decoder mismatches, race tests, negative compilation, adversaries, and benchmarks | ↗ request snapshots |
