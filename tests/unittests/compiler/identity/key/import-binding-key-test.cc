...
ImportBindingKey importKey(
  ...
  auto value = ImportBindingKey::from(
...
ZC_TEST("ImportBindingKey admits module alias slots") {
  auto alias =
...
ZC_TEST("ImportBindingKey rejects requester operation and namespace mismatches") {
  ZC_EXPECT(ImportBindingKey::from(
...
ZC_TEST("ImportBindingKey canonicalizes source and local names to NFC") {
  ...
  ZC_IF_SOME(decomposedValue, decomposed) {
    ZC_IF_SOME(composedValue, composed) {
      auto first = ImportBindingKey::from(
          module("app"_zc), resolution("app"_zc, ModuleDependencyKind::Import, "dep"_zc),
      ...
      auto second = ImportBindingKey::from(
          module("app"_zc), resolution("app"_zc, ModuleDependencyKind::Import, "dep"_zc),
