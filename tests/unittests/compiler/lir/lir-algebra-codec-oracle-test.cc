...
ZC_TEST("LIR algebra empty registry reproduces the RFC 0021 56-byte oracle") {
  const auto registry = AlgebraRegistry::empty();
  ...
  auto bytes = AlgebraCodec::encode(registry);
  ...
  auto revision = AlgebraCodec::compute(registry);
...
ZC_TEST("LIR algebra revision is field sensitive") {
  const auto baseDigest = AlgebraCodec::compute(AlgebraRegistry::empty()).digest();

  {
    // A different source-MIR revision domain changes the digest.
    auto altered = AlgebraRegistry::withSourceDomain("zom.mir-revision-x"_zc);
    ZC_REQUIRE(altered != zc::none);
    ZC_IF_SOME(registry, altered) {
      ZC_EXPECT(AlgebraCodec::compute(registry).digest() != baseDigest);
    }
  }
  {
    // One appended source recipe changes the digest (count and framed record).
    auto registry = AlgebraRegistry::empty();
  ...
  }
  {
    ...
    auto asSource = AlgebraRegistry::empty();
    auto asGenerated = AlgebraRegistry::empty();
    const uint8_t record[] = {0x07, 0x07};
    asSource.addRecipe(zc::arrayPtr(record, 2));
    asGenerated.addGeneratedRecipe(zc::arrayPtr(record, 2));
    ZC_EXPECT(AlgebraCodec::compute(asSource).digest() !=
              AlgebraCodec::compute(asGenerated).digest());
  }
}
...
ZC_TEST("LIR algebra codec is deterministic and domain framed") {
  auto first = AlgebraCodec::encode(AlgebraRegistry::empty());
  auto second = AlgebraCodec::encode(AlgebraRegistry::empty());
...
ZC_TEST("LIR algebra registry rejects an invalid source domain") {
  ZC_EXPECT(AlgebraRegistry::withSourceDomain(""_zc) == zc::none);
  ZC_EXPECT(AlgebraRegistry::withSourceDomain("zom.\xff"_zc) == zc::none);
}
