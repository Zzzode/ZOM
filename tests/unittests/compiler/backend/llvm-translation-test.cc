...
ZC_TEST("Whole-struct return slots follow source-literal element order, not a sort") {
  ...
  ZC_EXPECT(lir::TranslationValidator::validate(function, ZC_REQUIRE_NONNULL(lir),
...
ZC_TEST("Scalar module initializers of non-i32 integer widths lower to a verified object") {
  ...
  for (const auto& widthCase : cases) {
    ...
    const auto& lirModule = ZC_REQUIRE_NONNULL(lir);
    ZC_EXPECT(lir::LirStructuralVerifier::verify(lirModule) == zc::none);
    ZC_EXPECT(lir::TranslationValidator::validate(function, lirModule,
                                                  typeContext.semanticTypes()) == zc::none);
    ZC_EXPECT(lirModule.functions().size() == 1);
...
// + Return(result). The call argument is a copy of the parameter slot.
mir::MirFunction buildParameterArgumentReturnCaller(identity::DefId owner, identity::DefId callee,
                                                    identity::SemanticTypeId i32) {
  zc::Vector<mir::MirSourceScope> scopes;
  scopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
  zc::Vector<mir::MirLocalDeclaration> locals;
  ...
  zc::Vector<mir::MirBasicBlock> blocks;
  // Entry: StorageLive(temporary); Call(id, [copy param 1], dest=temporary, bb2).
  zc::Vector<mir::MirStatement> entryStatements;
  entryStatements.add(mir::MirStatement::storageLive(localId(2), span()));
  zc::Vector<mir::MirOperand> arguments;
  arguments.add(mir::MirOperand::copy(resultPlace(localId(1), i32)));
  blocks.add(mir::MirBasicBlock{
      ...
      mir::MirTerminator::call(callee, zc::mv(arguments), mir::MirCallEffect::noActivation(),
                               resultPlace(localId(2), i32), blockId(2), zc::none, span())});
  // Continuation: materialize the function result from the temporary and return it.
  zc::Vector<mir::MirStatement> continuationStatements;
  continuationStatements.add(mir::MirStatement::storageLive(localId(3), span()));
  continuationStatements.add(mir::MirStatement::assign(
      resultPlace(localId(3), i32),
      mir::MirRvalue::use(mir::MirOperand::move(resultPlace(localId(2), i32))),
      mir::MirInitializationKind::Initialize, span()));
  continuationStatements.add(mir::MirStatement::storageDead(localId(2), span()));
  blocks.add(mir::MirBasicBlock{blockId(2), scopeId(1), zc::mv(continuationStatements),
                                mir::MirTerminator::returnValue(
                                    mir::MirOperand::copy(resultPlace(localId(3), i32)), span())});
...
ZC_TEST("Same-module return call forwarding its parameter argument lowers to verified LLVM") {
  tests::TestSemanticTypeContext typeContext;
  const auto i32 = typeContext.internPrimitive(type::semantic::PrimitiveKind::I32);
  const auto callerOwner = tests::testDefinition(2);
  const auto calleeOwner = tests::testDefinition(3);

  auto callee = buildParameterReturnCallee(calleeOwner, i32);
  auto caller = buildParameterArgumentReturnCaller(callerOwner, calleeOwner, i32);
  ...
  const auto ir = result.textualIr();
  ZC_EXPECT(ir.contains("call i32"_zc));
  // The argument is the incoming parameter slot, never an integer constant.
  ZC_EXPECT(ir.contains("load"_zc));
  ZC_EXPECT(!ir.contains("(i32 0)"_zc));
...
// Shared-receiver inherent method call: a Method-sourced callee with one
// shared-reference receiver parameter returning a scalar constant 7, and a
// caller that initializes a one-field owner local, takes its address, calls the
// method with that pointer, and returns the result. Lowers to a verified LLVM
// module (`call i32 @zom.callee(ptr)`, `alloca ptr`, `ret i32 7`) and an ELF
// object; the caller folds to the runnable `zom.module_init` entry.
namespace {
identity::SemanticTypeId internTypeData(type::SemanticTypeStore& types,
                                        type::semantic::TypeData data) {
  auto canonical = types.canonicalizeClosed(zc::mv(data));
  ZC_REQUIRE(canonical.is<type::semantic::CanonicalTypeData>());
  auto interned = types.intern(zc::mv(canonical).get<type::semantic::CanonicalTypeData>());
  ZC_REQUIRE(interned.is<type::SemanticTypeInterned>());
  return interned.get<type::SemanticTypeInterned>().id;
}
}  // namespace

// Builds the callee half of a shared-receiver call. When fieldRead is true the
// callee returns a copy place-use of its receiver parameter reached through
// [Dereference, Field]; otherwise it returns a scalar constant.
mir::MirFunction buildSharedReceiverCallee(identity::DefId calleeOwner,
                                           identity::SemanticTypeId i32,
                                           identity::SemanticTypeId receiverType,
                                           identity::DefId fieldDef, bool fieldRead) {
  zc::Vector<mir::MirSourceScope> scopes;
  scopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
  zc::Vector<mir::MirLocalDeclaration> locals;
  locals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::Parameter, receiverType,
                                      scopeId(1), span()});
  zc::Vector<mir::MirStatement> statements;
  mir::MirOperand returnOperand =
      mir::MirOperand::constant(i32, integerConstant(fieldRead ? 42 : 7));
  if (fieldRead) {
    zc::Vector<mir::MirProjection> projections;
    projections.add(mir::MirProjection::dereference(receiverType, i32));
    projections.add(mir::MirProjection::field(fieldDef, i32, i32));
    returnOperand =
        mir::MirOperand::copy(mir::MirPlace(localId(1), receiverType, zc::mv(projections), i32));
  }
  zc::Vector<mir::MirBasicBlock> blocks;
  blocks.add(mir::MirBasicBlock{blockId(1), scopeId(1), zc::mv(statements),
...
// Builds one mutable-receiver callee that overwrites the receiver field with a
// constant and returns the same field: one projected Overwrite Assign followed
// by a [Dereference, Field] place-use return.
mir::MirFunction buildReceiverFieldWriteCallee(identity::DefId calleeOwner,
                                               identity::SemanticTypeId i32,
                                               identity::SemanticTypeId receiverType,
                                               identity::SemanticTypeId cellType,
                                               identity::DefId fieldDef) {
  zc::Vector<mir::MirSourceScope> scopes;
  scopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
  zc::Vector<mir::MirLocalDeclaration> locals;
  locals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::Parameter, receiverType,
  ...
  zc::Vector<mir::MirStatement> statements;
  {
    zc::Vector<mir::MirProjection> writeProjections;
    writeProjections.add(mir::MirProjection::dereference(receiverType, cellType));
    writeProjections.add(mir::MirProjection::field(fieldDef, cellType, i32));
    statements.add(mir::MirStatement::assign(
        mir::MirPlace(localId(1), receiverType, zc::mv(writeProjections), i32),
  ...
  returnProjections.add(mir::MirProjection::dereference(receiverType, cellType));
  returnProjections.add(mir::MirProjection::field(fieldDef, cellType, i32));
  auto returnOperand = mir::MirOperand::copy(
      mir::MirPlace(localId(1), receiverType, zc::mv(returnProjections), i32));
  blocks.add(mir::MirBasicBlock{blockId(1), scopeId(1), zc::mv(statements),
                                mir::MirTerminator::returnValue(zc::mv(returnOperand), span())});
  return mir::MirFunction{calleeOwner,
                          mir::MirFunctionKind::Function,
...
ZC_TEST("Shared-receiver method call lowers MIR -> LIR -> verified LLVM with a pointer argument") {
  ...
  // The pointer lowering is referent-agnostic; a shared reference to the folded
  // element carrier exercises the exact same LIR/LLVM pointer path as `&Cell`.
  ...
  // Callee: one receiver parameter local, single block returning constant 7.
  {
    mir::MirFunction callee =
        buildSharedReceiverCallee(calleeOwner, i32, receiverType, fieldDef, /*fieldRead=*/false);
    ...
    callerLocals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::UserLocal, cellType,
                                              scopeId(1), span()});
    callerLocals.add(mir::MirLocalDeclaration{localId(2), mir::MirLocalKind::Temporary,
                                              receiverType, scopeId(1), span()});
    callerLocals.add(mir::MirLocalDeclaration{localId(3), mir::MirLocalKind::Temporary, i32,
                                              scopeId(1), span()});
    zc::Vector<mir::MirStatement> entry;
    entry.add(mir::MirStatement::storageLive(localId(1), span()));
    zc::Vector<mir::MirNominalAggregateElement> elements;
    elements.add(mir::MirNominalAggregateElement{
        fieldDef, mir::MirOperand::constant(i32, integerConstant(0))});
    {
      zc::Vector<mir::MirProjection> projections;
    ...
    ZC_EXPECT(lir::LirStructuralVerifier::verify(lirModule) == zc::none);
    {
      ...
      functions[0] = &caller;
      functions[1] = &callee;
      ZC_EXPECT(lir::TranslationValidator::validate(functions.asPtr(), lirModule, types) ==
                zc::none);
    }
    ...
    const auto ir = result.textualIr();
    ZC_EXPECT(ir.contains("call i32"_zc));
    ZC_EXPECT(ir.contains("@zom.callee(ptr"_zc));
    ZC_EXPECT(ir.contains("alloca ptr"_zc));
    ZC_EXPECT(ir.contains("zom.module_init"_zc));
    ZC_EXPECT(ir.contains("ret i32 7"_zc));
...
ZC_TEST("Shared-receiver self-call chain lowers MIR -> LIR -> verified LLVM") {
  tests::TestSemanticTypeContext typeContext;
  const auto i32 = typeContext.internPrimitive(type::semantic::PrimitiveKind::I32);
  const auto structDef = tests::testDefinition(2);
  const auto fieldDef = tests::testDefinition(3);
  const auto callerOwner = tests::testDefinition(0);
  const auto forwarderOwner = tests::testDefinition(1);
  const auto leafOwner = tests::testDefinition(4);
  auto& types = typeContext.semanticTypes();
  const auto cellType = i32;
  const auto receiverType =
      internTypeData(types, type::semantic::TypeData(type::semantic::ReferenceTypeData{
  ...
  // Leaf: a shared-receiver Method with one pointer parameter whose single
  // block returns constant 42. Built directly so the constant is independent
  // of the fieldRead helper default.
  mir::MirFunction leaf = [&] {
    zc::Vector<mir::MirSourceScope> scopes;
    scopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
    zc::Vector<mir::MirLocalDeclaration> locals;
    locals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::Parameter, receiverType,
    ...
    zc::Vector<mir::MirBasicBlock> blocks;
    blocks.add(
        mir::MirBasicBlock{blockId(1), scopeId(1), zc::mv(statements),
                           mir::MirTerminator::returnValue(
                               mir::MirOperand::constant(i32, integerConstant(42)), span())});
    return mir::MirFunction{leafOwner,
                            mir::MirFunctionKind::Function,
                            identity::DefinitionKind::Method,
                            i32,
                            span(),
  ...
  // Forwarder: one receiver Parameter and one result Temporary; storage-live the
  // result, call the leaf forwarding the receiver local, then return it.
  mir::MirFunction forwarder = [&] {
    zc::Vector<mir::MirSourceScope> scopes;
    scopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
    zc::Vector<mir::MirLocalDeclaration> locals;
    locals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::Parameter, receiverType,
                                        scopeId(1), span()});
    locals.add(mir::MirLocalDeclaration{localId(2), mir::MirLocalKind::Temporary, i32, scopeId(1),
                                        span()});
    zc::Vector<mir::MirStatement> entry;
    entry.add(mir::MirStatement::storageLive(localId(2), span()));
    zc::Vector<mir::MirOperand> arguments;
    {
      zc::Vector<mir::MirProjection> projections;
      arguments.add(mir::MirOperand::copy(
          mir::MirPlace(localId(1), receiverType, zc::mv(projections), receiverType)));
    }
    zc::Vector<mir::MirBasicBlock> blocks;
    blocks.add(mir::MirBasicBlock{
        blockId(1), scopeId(1), zc::mv(entry),
        mir::MirTerminator::call(leafOwner, zc::mv(arguments), mir::MirCallEffect::noActivation(),
                                 resultPlace(localId(2), i32), blockId(2), zc::none, span())});
    zc::Vector<mir::MirStatement> continuation;
    blocks.add(
        mir::MirBasicBlock{blockId(2), scopeId(1), zc::mv(continuation),
                           mir::MirTerminator::returnValue(
                               mir::MirOperand::move(resultPlace(localId(2), i32)), span())});
    return mir::MirFunction{forwarderOwner,
                            mir::MirFunctionKind::Function,
                            identity::DefinitionKind::Method,
                            i32,
                            span(),
  ...
  // Caller: the standard owner/borrow/result receiver-call shape targeting the
  // forwarder.
  mir::MirFunction caller = [&] {
    ...
    callerBlocks.add(mir::MirBasicBlock{
        blockId(1), scopeId(1), zc::mv(entry),
        mir::MirTerminator::call(forwarderOwner, zc::mv(arguments),
                                 mir::MirCallEffect::noActivation(), resultPlace(localId(3), i32),
                                 blockId(2), zc::none, span())});
    zc::Vector<mir::MirStatement> continuation;
    callerBlocks.add(
        mir::MirBasicBlock{blockId(2), scopeId(1), zc::mv(continuation),
                           mir::MirTerminator::returnValue(
                               mir::MirOperand::move(resultPlace(localId(3), i32)), span())});
    return mir::MirFunction{callerOwner,
                            mir::MirFunctionKind::Function,
                            identity::DefinitionKind::Function,
                            i32,
                            span(),
  ...
  auto lir = lir::MirToLirLowering::lowerReceiverSelfCallModule(caller, forwarder, leaf, types);
  ZC_REQUIRE(lir != zc::none);
  const auto& lirModule = ZC_REQUIRE_NONNULL(lir);
  ZC_EXPECT(lir::LirStructuralVerifier::verify(lirModule) == zc::none);
  {
  ...
  const auto ir = result.textualIr();
  ZC_EXPECT(ir.contains("zom.module_init"_zc));
  ZC_EXPECT(ir.contains("zom.forwarder"_zc));
  ZC_EXPECT(ir.contains("zom.leaf"_zc));
  // Exactly two calls: module_init -> forwarder, forwarder -> leaf.
  ZC_EXPECT(countOccurrences(ir, "call i32"_zc) == 2);
  ZC_EXPECT(ir.contains("call i32 @zom.forwarder(ptr"_zc));
  ZC_EXPECT(ir.contains("call i32 @zom.leaf(ptr"_zc));
  ZC_EXPECT(ir.contains("ret i32 42"_zc));
...
ZC_TEST("Shared-receiver field read lowers MIR -> LIR -> verified LLVM through a pointer load") {
  tests::TestSemanticTypeContext typeContext;
  const auto i32 = typeContext.internPrimitive(type::semantic::PrimitiveKind::I32);
  const auto structDef = tests::testDefinition(2);
  const auto fieldDef = tests::testDefinition(3);
  ...
  mir::MirFunction callee = buildSharedReceiverCallee(calleeOwner, i32, receiverType, fieldDef,
                                                      /*fieldRead=*/true);

  // Caller mirrors the pointer-argument test but stores the field value 42.
  zc::Vector<mir::MirSourceScope> callerScopes;
  callerScopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
  zc::Vector<mir::MirLocalDeclaration> callerLocals;
  callerLocals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::UserLocal, cellType,
  ...
  zc::Vector<mir::MirNominalAggregateElement> elements;
  elements.add(mir::MirNominalAggregateElement{
      fieldDef, mir::MirOperand::constant(i32, integerConstant(42))});
  {
    zc::Vector<mir::MirProjection> projections;
    entry.add(mir::MirStatement::assign(
        mir::MirPlace(localId(1), cellType, zc::mv(projections), cellType),
  ...
  ZC_EXPECT(ir.contains("@zom.callee(ptr"_zc));
  ZC_EXPECT(ir.contains("zom.module_init"_zc));
  // The callee loads the receiver pointer then the field; it has no literal
  // return.
  ZC_EXPECT(ir.contains("load ptr, ptr"_zc));
  ZC_EXPECT(ir.contains("load i32, ptr"_zc));
  ZC_EXPECT(!ir.contains("ret i32 7"_zc));
...
ZC_TEST("Shared-receiver method with one integer argument returns the argument") {
  tests::TestSemanticTypeContext typeContext;
  const auto i32 = typeContext.internPrimitive(type::semantic::PrimitiveKind::I32);
  const auto structDef = tests::testDefinition(2);
  const auto fieldDef = tests::testDefinition(3);
  ...
  // Callee: receiver parameter local 1 (pointer) and explicit i32 parameter
  // local 2; one empty block returning a bare place-use of local 2.
  zc::Vector<mir::MirSourceScope> calleeScopes;
  calleeScopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
  zc::Vector<mir::MirLocalDeclaration> calleeLocals;
  calleeLocals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::Parameter, receiverType,
                                            scopeId(1), span()});
  calleeLocals.add(
      mir::MirLocalDeclaration{localId(2), mir::MirLocalKind::Parameter, i32, scopeId(1), span()});
  zc::Vector<mir::MirStatement> calleeStatements;
  {
    zc::Vector<mir::MirProjection> projections;
    zc::Vector<mir::MirBasicBlock> calleeBlocks;
    calleeBlocks.add(mir::MirBasicBlock{
        blockId(1), scopeId(1), zc::mv(calleeStatements),
        mir::MirTerminator::returnValue(
            mir::MirOperand::copy(mir::MirPlace(localId(2), i32, zc::mv(projections), i32)),
            span())});
    ...
    // carries the receiver pointer and the constant argument 42.
    zc::Vector<mir::MirSourceScope> callerScopes;
    callerScopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
    zc::Vector<mir::MirLocalDeclaration> callerLocals;
    callerLocals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::UserLocal, cellType,
    ...
    zc::Vector<mir::MirNominalAggregateElement> elements;
    elements.add(mir::MirNominalAggregateElement{
        fieldDef, mir::MirOperand::constant(i32, integerConstant(7))});
    {
      zc::Vector<mir::MirProjection> projections;
      entry.add(mir::MirStatement::assign(
          mir::MirPlace(localId(1), cellType, zc::mv(projections), cellType),
    ...
    arguments.add(mir::MirOperand::constant(i32, integerConstant(42)));
    zc::Vector<mir::MirBasicBlock> callerBlocks;
    callerBlocks.add(mir::MirBasicBlock{
        blockId(1), scopeId(1), zc::mv(entry),
        mir::MirTerminator::call(calleeOwner, zc::mv(arguments), mir::MirCallEffect::noActivation(),
    ...
    const auto ir = result.textualIr();
    ZC_EXPECT(ir.contains("call i32"_zc));
    ZC_EXPECT(ir.contains("@zom.callee(ptr %0, i32 %1)"_zc));
    ZC_EXPECT(ir.contains("i32 42"_zc));
    ZC_EXPECT(ir.contains("zom.module_init"_zc));
    // The callee returns its incoming argument slot, not a literal.
    ZC_EXPECT(!ir.contains("ret i32 42"_zc));
...
ZC_TEST("Shared-receiver method with parameter arithmetic returns the computed result") {
  tests::TestSemanticTypeContext typeContext;
  const auto i32 = typeContext.internPrimitive(type::semantic::PrimitiveKind::I32);
  const auto structDef = tests::testDefinition(2);
  const auto fieldDef = tests::testDefinition(3);
  ...
  // Callee: receiver pointer local 1, ordinary i32 parameter local 2, and
  // FunctionResult local 3; StorageLive(3), 3 = copy 2 Add const 1; return 3.
  zc::Vector<mir::MirSourceScope> calleeScopes;
  calleeScopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
  zc::Vector<mir::MirLocalDeclaration> calleeLocals;
  calleeLocals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::Parameter, receiverType,
  ...
  calleeLocals.add(
      mir::MirLocalDeclaration{localId(2), mir::MirLocalKind::Parameter, i32, scopeId(1), span()});
  calleeLocals.add(mir::MirLocalDeclaration{localId(3), mir::MirLocalKind::FunctionResult, i32,
                                            scopeId(1), span()});
  zc::Vector<mir::MirStatement> calleeStatements;
  calleeStatements.add(mir::MirStatement::storageLive(localId(3), span()));
  {
    zc::Vector<mir::MirProjection> projections;
    calleeStatements.add(mir::MirStatement::assign(
        mir::MirPlace(localId(3), i32, zc::mv(projections), i32),
        mir::MirRvalue::arithmetic(mir::MirArithmeticOperator::Add,
                                   mir::MirOperand::copy(resultPlace(localId(2), i32)),
                                   mir::MirOperand::constant(i32, integerConstant(1)), i32),
        mir::MirInitializationKind::Initialize, span()));
  }
  zc::Vector<mir::MirBasicBlock> calleeBlocks;
  calleeBlocks.add(
      mir::MirBasicBlock{blockId(1), scopeId(1), zc::mv(calleeStatements),
                         mir::MirTerminator::returnValue(
                             mir::MirOperand::move(resultPlace(localId(3), i32)), span())});
  mir::MirFunction callee{calleeOwner,
                          mir::MirFunctionKind::Function,
  ...
  // Caller: owner local, shared borrow temporary, result temporary; the call
  // carries the receiver pointer and the constant argument 41.
  zc::Vector<mir::MirSourceScope> callerScopes;
  callerScopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
  zc::Vector<mir::MirLocalDeclaration> callerLocals;
  callerLocals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::UserLocal, cellType,
  ...
  arguments.add(mir::MirOperand::constant(i32, integerConstant(41)));
  zc::Vector<mir::MirBasicBlock> callerBlocks;
  callerBlocks.add(mir::MirBasicBlock{
      blockId(1), scopeId(1), zc::mv(entry),
      mir::MirTerminator::call(calleeOwner, zc::mv(arguments), mir::MirCallEffect::noActivation(),
  ...
  ZC_EXPECT(ir.contains("call i32"_zc));
  ZC_EXPECT(ir.contains("@zom.callee(ptr %0, i32 %1)"_zc));
  ZC_EXPECT(ir.contains("i32 41"_zc));
...
ZC_TEST("Shared-receiver method with receiver-field arithmetic returns the computed result") {
  tests::TestSemanticTypeContext typeContext;
  const auto i32 = typeContext.internPrimitive(type::semantic::PrimitiveKind::I32);
  const auto structDef = tests::testDefinition(2);
  const auto fieldDef = tests::testDefinition(3);
  ...
  // Callee: receiver pointer local 1 and FunctionResult local 2;
  // StorageLive(2), 2 = (copy [Dereference, Field] local 1) Add const 1;
  // return 2.
  zc::Vector<mir::MirSourceScope> calleeScopes;
  calleeScopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
  zc::Vector<mir::MirLocalDeclaration> calleeLocals;
  calleeLocals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::Parameter, receiverType,
                                            scopeId(1), span()});
  calleeLocals.add(mir::MirLocalDeclaration{localId(2), mir::MirLocalKind::FunctionResult, i32,
                                            scopeId(1), span()});
  zc::Vector<mir::MirStatement> calleeStatements;
  calleeStatements.add(mir::MirStatement::storageLive(localId(2), span()));
  {
    zc::Vector<mir::MirProjection> projections;
    projections.add(mir::MirProjection::dereference(receiverType, cellType));
    projections.add(mir::MirProjection::field(fieldDef, cellType, i32));
    auto fieldOperand =
        mir::MirOperand::copy(mir::MirPlace(localId(1), receiverType, zc::mv(projections), i32));
    calleeStatements.add(mir::MirStatement::assign(
        mir::MirPlace(localId(2), i32, {}, i32),
        mir::MirRvalue::arithmetic(mir::MirArithmeticOperator::Add, zc::mv(fieldOperand),
                                   mir::MirOperand::constant(i32, integerConstant(1)), i32),
        mir::MirInitializationKind::Initialize, span()));
  }
  zc::Vector<mir::MirBasicBlock> calleeBlocks;
  ...
  // Caller: owner local initialized to 41, shared borrow temporary, result
  // temporary; the call carries only the receiver pointer.
  zc::Vector<mir::MirSourceScope> callerScopes;
  callerScopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
  zc::Vector<mir::MirLocalDeclaration> callerLocals;
  callerLocals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::UserLocal, cellType,
  ...
  zc::Vector<mir::MirNominalAggregateElement> elements;
  elements.add(mir::MirNominalAggregateElement{
      fieldDef, mir::MirOperand::constant(i32, integerConstant(41))});
  {
    zc::Vector<mir::MirProjection> projections;
    entry.add(mir::MirStatement::assign(
        mir::MirPlace(localId(1), cellType, zc::mv(projections), cellType),
  ...
  const auto ir = result.textualIr();
  // The callee loads the receiver field and increments it; the call carries
  // only the receiver pointer and there is no ordinary i32 argument.
  ZC_EXPECT(ir.contains("load i32, ptr"_zc));
  ZC_EXPECT(ir.contains("add i32"_zc));
  ZC_EXPECT(ir.contains("@zom.callee(ptr %0)"_zc));
  ZC_EXPECT(!ir.contains("@zom.callee(ptr %0, i32"_zc));
  ZC_EXPECT(ir.contains("i32 41"_zc));
...
ZC_TEST(
    "Mutating-receiver method call lowers MIR -> LIR -> verified LLVM with a pointer argument") {
  tests::TestSemanticTypeContext typeContext;
  const auto i32 = typeContext.internPrimitive(type::semantic::PrimitiveKind::I32);
  const auto structDef = tests::testDefinition(2);
  const auto fieldDef = tests::testDefinition(3);
  ...
  const auto receiverType =
      internTypeData(types, type::semantic::TypeData(type::semantic::ReferenceTypeData{
                                type::semantic::Mutability::Mutable, cellType}));

  // Callee: one mutable receiver parameter local, single block returning
  // constant 7.
  mir::MirFunction callee =
      buildSharedReceiverCallee(calleeOwner, i32, receiverType, fieldDef, /*fieldRead=*/false);

  // Caller: owner local, mutable borrow temporary, result temporary; the call
  // effect activates the receiver borrow temporary.
  zc::Vector<mir::MirSourceScope> callerScopes;
  callerScopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
  zc::Vector<mir::MirLocalDeclaration> callerLocals;
  callerLocals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::UserLocal, cellType,
  ...
  callerBlocks.add(mir::MirBasicBlock{
      blockId(1), scopeId(1), zc::mv(entry),
      mir::MirTerminator::call(calleeOwner, zc::mv(arguments),
                               mir::MirCallEffect::activateMutableReceiver(localId(2)),
                               resultPlace(localId(3), i32), blockId(2), zc::none, span())});
  zc::Vector<mir::MirStatement> continuation;
  callerBlocks.add(
      mir::MirBasicBlock{blockId(2), scopeId(1), zc::mv(continuation),
...
ZC_TEST("Mutating-receiver field write-read lowers StoreField then LoadField") {
  tests::TestSemanticTypeContext typeContext;
  const auto i32 = typeContext.internPrimitive(type::semantic::PrimitiveKind::I32);
  const auto structDef = tests::testDefinition(2);
  const auto fieldDef = tests::testDefinition(3);
  ...
  // Callee: one mutable receiver parameter, one projected overwrite of the
  // field, one projected field read return.
  mir::MirFunction callee =
      buildReceiverFieldWriteCallee(calleeOwner, i32, receiverType, cellType, fieldDef);
  ...
  const auto ir = result.textualIr();
  ZC_EXPECT(ir.contains("@zom.callee"_zc));
  // The write statement stores the immediate through the receiver pointer, and
  // the synthesized result slot then loads the same field.
  ZC_EXPECT(ir.contains("store i32 42, ptr"_zc));
  ZC_EXPECT(ir.contains("ret i32"_zc));
...
ZC_TEST("Mutating unit receiver call lowers a void call before the field read") {
  // The void vertical: the caller aggregate-initializes one owner local, invokes
  // the mutating unit setter (its parameter is stored through the receiver and
  // it returns void), then invokes the shared getter and returns the field. The
  // MIR unit call-destination temporary is dropped by dense LIR renumbering, so
  // the setter call has no SSA result and lowers to `call void` / `ret void`.
  tests::TestSemanticTypeContext typeContext;
  const auto i32 = typeContext.internPrimitive(type::semantic::PrimitiveKind::I32);
  const auto unit = typeContext.internPrimitive(type::semantic::PrimitiveKind::Unit);
  const auto structDef = tests::testDefinition(2);
  const auto fieldDef = tests::testDefinition(3);
  const auto callerOwner = tests::testDefinition(0);
  const auto setterOwner = tests::testDefinition(1);
  const auto getterOwner = tests::testDefinition(4);
  auto& types = typeContext.semanticTypes();
  const auto cellType = i32;
  const auto mutableReceiverType =
      internTypeData(types, type::semantic::TypeData(type::semantic::ReferenceTypeData{
                                type::semantic::Mutability::Mutable, cellType}));
  const auto sharedReceiverType =
      internTypeData(types, type::semantic::TypeData(type::semantic::ReferenceTypeData{
                                type::semantic::Mutability::Const, cellType}));

  // Setter: mutable receiver plus i32 parameter, one projected Overwrite Assign
  // copying the parameter into the field, then a value-less unit Return.
  mir::MirFunction setter = [&] {
    zc::Vector<mir::MirSourceScope> scopes;
    scopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
    zc::Vector<mir::MirLocalDeclaration> locals;
    locals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::Parameter,
                                        mutableReceiverType, scopeId(1), span()});
    locals.add(mir::MirLocalDeclaration{localId(2), mir::MirLocalKind::Parameter, i32, scopeId(1),
                                        span()});
    zc::Vector<mir::MirStatement> statements;
    {
      zc::Vector<mir::MirProjection> writeProjections;
      writeProjections.add(mir::MirProjection::dereference(mutableReceiverType, cellType));
      writeProjections.add(mir::MirProjection::field(fieldDef, cellType, i32));
      zc::Vector<mir::MirProjection> rhsProjections;
      statements.add(mir::MirStatement::assign(
          mir::MirPlace(localId(1), mutableReceiverType, zc::mv(writeProjections), i32),
          mir::MirRvalue::use(
              mir::MirOperand::copy(mir::MirPlace(localId(2), i32, zc::mv(rhsProjections), i32))),
          mir::MirInitializationKind::Overwrite, span()));
    }
    zc::Vector<mir::MirBasicBlock> blocks;
    blocks.add(mir::MirBasicBlock{blockId(1), scopeId(1), zc::mv(statements),
                                  mir::MirTerminator::returnVoid(span())});
    return mir::MirFunction{setterOwner,
                            mir::MirFunctionKind::Function,
                            identity::DefinitionKind::Method,
                            unit,
                            span(),
  ...
  // Getter: shared receiver, zero statements, projected field-copy return.
  mir::MirFunction getter =
      buildSharedReceiverCallee(getterOwner, i32, sharedReceiverType, fieldDef,
                                /*fieldRead=*/true);

  // Caller: five MIR locals across three blocks; the unit destination temporary
  // (local 3) is dropped during lowering.
  zc::Vector<mir::MirSourceScope> callerScopes;
  callerScopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
  zc::Vector<mir::MirLocalDeclaration> callerLocals;
  callerLocals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::UserLocal, cellType,
  ...
  callerLocals.add(mir::MirLocalDeclaration{localId(2), mir::MirLocalKind::Temporary,
                                            mutableReceiverType, scopeId(1), span()});
  callerLocals.add(
      mir::MirLocalDeclaration{localId(3), mir::MirLocalKind::Temporary, unit, scopeId(1), span()});
  callerLocals.add(mir::MirLocalDeclaration{localId(4), mir::MirLocalKind::Temporary,
                                            sharedReceiverType, scopeId(1), span()});
  callerLocals.add(
      mir::MirLocalDeclaration{localId(5), mir::MirLocalKind::Temporary, i32, scopeId(1), span()});
  ...
  zc::Vector<mir::MirBasicBlock> callerBlocks;
  {
    ...
    elements.add(mir::MirNominalAggregateElement{
        fieldDef, mir::MirOperand::constant(i32, integerConstant(0))});
    entry.add(mir::MirStatement::assign(
        resultPlace(localId(1), cellType),
        mir::MirRvalue::nominalAggregate(structDef, cellType, zc::mv(elements)),
        mir::MirInitializationKind::Initialize, span()));
    entry.add(mir::MirStatement::storageLive(localId(2), span()));
    entry.add(mir::MirStatement::borrowCreation(
        mir::MirPlace(localId(2), mutableReceiverType, zc::Vector<mir::MirProjection>{},
                      mutableReceiverType),
        mir::MirBorrowKind::Mutable,
        mir::MirPlace(localId(1), cellType, zc::Vector<mir::MirProjection>{}, cellType), span()));
    entry.add(mir::MirStatement::storageLive(localId(3), span()));
    zc::Vector<mir::MirOperand> arguments;
    arguments.add(mir::MirOperand::copy(mir::MirPlace(
        localId(2), mutableReceiverType, zc::Vector<mir::MirProjection>{}, mutableReceiverType)));
    arguments.add(mir::MirOperand::constant(i32, integerConstant(42)));
    callerBlocks.add(mir::MirBasicBlock{
        blockId(1), scopeId(1), zc::mv(entry),
        mir::MirTerminator::call(setterOwner, zc::mv(arguments),
                                 mir::MirCallEffect::activateMutableReceiver(localId(2)),
                                 resultPlace(localId(3), unit), blockId(2), zc::none, span())});
  }
  {
    zc::Vector<mir::MirStatement> middle;
    middle.add(mir::MirStatement::storageLive(localId(4), span()));
    middle.add(mir::MirStatement::borrowCreation(
        mir::MirPlace(localId(4), sharedReceiverType, zc::Vector<mir::MirProjection>{},
                      sharedReceiverType),
        mir::MirBorrowKind::Shared,
        mir::MirPlace(localId(1), cellType, zc::Vector<mir::MirProjection>{}, cellType), span()));
    middle.add(mir::MirStatement::storageLive(localId(5), span()));
    zc::Vector<mir::MirOperand> arguments;
    arguments.add(mir::MirOperand::copy(mir::MirPlace(
        localId(4), sharedReceiverType, zc::Vector<mir::MirProjection>{}, sharedReceiverType)));
    callerBlocks.add(mir::MirBasicBlock{
        blockId(2), scopeId(1), zc::mv(middle),
        mir::MirTerminator::call(getterOwner, zc::mv(arguments), mir::MirCallEffect::noActivation(),
                                 resultPlace(localId(5), i32), blockId(3), zc::none, span())});
  }
  callerBlocks.add(
      mir::MirBasicBlock{blockId(3), scopeId(1), zc::Vector<mir::MirStatement>{},
                         mir::MirTerminator::returnValue(
                             mir::MirOperand::move(resultPlace(localId(5), i32)), span())});
  mir::MirFunction caller{callerOwner,
                          mir::MirFunctionKind::Function,
                          identity::DefinitionKind::Function,
                          i32,
  ...
  auto lir =
      lir::MirToLirLowering::lowerReceiverVoidThenValueCallModule(caller, setter, getter, types);
  ZC_REQUIRE(lir != zc::none);
  const auto& lirModule = ZC_REQUIRE_NONNULL(lir);
  ZC_EXPECT(lir::LirStructuralVerifier::verify(lirModule) == zc::none);
  {
  ...
  const auto ir = result.textualIr();
  // The setter call carries no SSA result and the setter returns void; its
  // parameter (42 at the call site) is stored through the receiver pointer.
  ZC_EXPECT(ir.contains("call void"_zc));
  ZC_EXPECT(ir.contains("@zom.setter"_zc));
  ZC_EXPECT(ir.contains("i32 42"_zc));
  ZC_EXPECT(ir.contains("ret void"_zc));
  // The getter loads the field and the entry returns its i32 result.
  ZC_EXPECT(ir.contains("call i32"_zc));
  ZC_EXPECT(ir.contains("@zom.getter"_zc));
  ZC_EXPECT(ir.contains("load i32"_zc));
  ZC_EXPECT(ir.contains("ret i32"_zc));
...
ZC_TEST("Mutating-receiver mismatches fail closed before LIR emission") {
  ...
  // Case 1: a shared caller borrow handed to a mutable-receiver callee is a
  // caller/callee mutability disagreement; the producer refuses to lower it.
  {
    mir::MirFunction callee = buildSharedReceiverCallee(calleeOwner, i32, mutableReceiverType,
                                                        fieldDef, /*fieldRead=*/false);
    zc::Vector<mir::MirSourceScope> callerScopes;
    callerScopes.add(mir::MirSourceScope{scopeId(1), zc::none, span()});
    zc::Vector<mir::MirLocalDeclaration> callerLocals;
    callerLocals.add(mir::MirLocalDeclaration{localId(1), mir::MirLocalKind::UserLocal, cellType,
    ...
    entry.add(mir::MirStatement::storageLive(localId(2), span()));
    {
      ...
      zc::Vector<mir::MirProjection> sourceProjections;
      entry.add(mir::MirStatement::borrowCreation(
          mir::MirPlace(localId(2), sharedReceiverType, zc::mv(destinationProjections),
                        sharedReceiverType),
          mir::MirBorrowKind::Shared,
          mir::MirPlace(localId(1), cellType, zc::mv(sourceProjections), cellType), span()));
    }
    ...
    zc::Vector<mir::MirOperand> arguments;
    {
      zc::Vector<mir::MirProjection> projections;
      arguments.add(mir::MirOperand::copy(
          mir::MirPlace(localId(2), sharedReceiverType, zc::mv(projections), sharedReceiverType)));
    }
    zc::Vector<mir::MirBasicBlock> callerBlocks;
    callerBlocks.add(mir::MirBasicBlock{
        blockId(1), scopeId(1), zc::mv(entry),
  ...
  // Case 2: a mutable receiver borrow whose call carries no activation agrees on
  // carrier but disagrees on the call effect; the independent validator rejects
  // the malformed pair even though the LIR module itself is structurally valid.
  {
    mir::MirFunction callee = buildSharedReceiverCallee(calleeOwner, i32, mutableReceiverType,
                                                        fieldDef, /*fieldRead=*/false);
    zc::Vector<mir::MirSourceScope> callerScopes;
    ...
    entry.add(mir::MirStatement::storageLive(localId(2), span()));
    {
      ...
      zc::Vector<mir::MirProjection> sourceProjections;
      entry.add(mir::MirStatement::borrowCreation(
          mir::MirPlace(localId(2), mutableReceiverType, zc::mv(destinationProjections),
                        mutableReceiverType),
          mir::MirBorrowKind::Mutable,
          mir::MirPlace(localId(1), cellType, zc::mv(sourceProjections), cellType), span()));
    }
    ...
    {
      zc::Vector<mir::MirProjection> projections;
      arguments.add(mir::MirOperand::copy(mir::MirPlace(localId(2), mutableReceiverType,
                                                        zc::mv(projections), mutableReceiverType)));
    }
    zc::Vector<mir::MirBasicBlock> callerBlocks;
    // The malformed part: mutable borrow, but NoActivation on the call.
    callerBlocks.add(mir::MirBasicBlock{
        blockId(1), scopeId(1), zc::mv(entry),
        mir::MirTerminator::call(calleeOwner, zc::mv(arguments), mir::MirCallEffect::noActivation(),
                                 resultPlace(localId(3), i32), blockId(2), zc::none, span())});
    ...
    // The producer must already refuse this disagreement.
    ZC_EXPECT(lir::MirToLirLowering::lowerReceiverCallModule(caller, callee, types) == zc::none);
  }
}
