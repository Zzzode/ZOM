...
impl<T> Trait for Box<T> {}
    ...
    auto verification = StableIdentityCandidateVerifier::verify(
  ...
  StableDefinitionHeader definitionHeader(zc::StringPtr name) const {
    ...
    return requireStableHeaderValue(StableDefinitionHeaderProducer::produce(
        StableDefinitionHeaderProductionInput{parsed(), query, entry, definitionSite(entry.key()),
                                              definitionSites(), implementationSites()}));
  }
  ...
  StableImplementationOccurrenceHeader implementationHeader(size_t ordinal) const {
    ...
    return requireStableHeaderValue(StableImplementationOccurrenceHeaderProducer::produce(
