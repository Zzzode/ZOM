...
struct HeaderFixture final {
  HeaderFixture(zc::StringPtr text, identity::SourceFileKey&& source)
    ...
    auto production = StableIdentityCandidateProducer::produce(
        parsed(), tests::test_identity_detail::module(), moduleNode);
    auto verified = StableIdentityCandidateVerifier::verify(
...
StableDefinitionHeader produce(const HeaderFixture& fixture, zc::StringPtr name) {
  ...
  return require(StableDefinitionHeaderProducer::produce(StableDefinitionHeaderProductionInput{
...
zc::Maybe<NamedDefinitionInventory> definitionsWithFlippedDisposition(
  ...
  auto verified = StableIdentityCandidateVerifier::reconstruct(
...
ZC_TEST("StableDefinitionHeaderProducer covers the complete header semantic matrix") {
...
ZC_TEST("StableDefinitionHeaderProducer maps module aliases to the import surface") {
...
ZC_TEST("StableDefinitionHeaderProducer selects the earliest equal source occurrence") {
  ...
  auto query =
      StableDefinitionQueryKey::from(tests::test_identity_detail::module(), entry.key().clone());
  ZC_EXPECT(StableDefinitionHeaderProducer::produce(StableDefinitionHeaderProductionInput{
...
ZC_TEST("StableDefinitionHeaderProducer rejects one-field cross-input mutations") {
  HeaderFixture fixture(matrixSource(), tests::test_identity_detail::source());
