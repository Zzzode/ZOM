...
zc::Maybe<CanonicalOverloadHeader> emptyHeader(CallableHeaderKind kind,
  ...
  return CanonicalOverloadHeader::from(kind, declaredName(), zc::mv(receiver), zc::mv(generics),
...
ZC_TEST("Canonical overload header passes the complete nine-field vector clone and accessors") {
  ...
  auto admitted = CanonicalOverloadHeader::from(
...
ZC_TEST("Canonical overload header rejects present-empty raises") {
  ...
  ZC_EXPECT(CanonicalOverloadHeader::from(CallableHeaderKind::Function, declaredName(),
