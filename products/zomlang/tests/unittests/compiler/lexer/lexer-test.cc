...
ZC_TEST("LexerTest.CommentHandling") {
  ...
  ZC_EXPECT(tokens[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[1].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[2].is(TokenKind::kEOF));
...
ZC_TEST("LexerTest.ErrorHandling") {
  ...
  ZC_EXPECT(tokens[0].is(TokenKind::kHash));
  ZC_EXPECT(tokens[1].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[2].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.FunctionKeywordAndIdentifier") {
  ...
  ZC_EXPECT(tokens[1].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[2].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[3].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[4].is(TokenKind::kEOF));
...
ZC_TEST("LexerTest.ComplexStringLiterals") {
  ...
  ZC_EXPECT(tokens[1].is(TokenKind::kStringLiteral));
  ZC_EXPECT(tokens[2].is(TokenKind::kStringLiteral));
  ZC_EXPECT(tokens[3].is(TokenKind::kStringLiteral));
  ZC_EXPECT(tokens[4].is(TokenKind::kEOF));
...
ZC_TEST("LexerTest.FunctionDeclarationTokens") {
  ...
  ZC_EXPECT(tokens[0].is(TokenKind::kFunKeyword));
  ZC_EXPECT(tokens[1].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[2].is(TokenKind::kLeftParen));
  ZC_EXPECT(tokens[3].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[4].is(TokenKind::kColon));
  ZC_EXPECT(tokens[5].is(TokenKind::kI32Keyword));
  ZC_EXPECT(tokens[6].is(TokenKind::kComma));
  ZC_EXPECT(tokens[7].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[8].is(TokenKind::kColon));
  ZC_EXPECT(tokens[9].is(TokenKind::kI32Keyword));
  ZC_EXPECT(tokens[10].is(TokenKind::kRightParen));
  ZC_EXPECT(tokens[11].is(TokenKind::kArrow));
  ZC_EXPECT(tokens[12].is(TokenKind::kI32Keyword));
  ZC_EXPECT(tokens[13].is(TokenKind::kLeftBrace));
  ZC_EXPECT(tokens[14].is(TokenKind::kReturnKeyword));
  ZC_EXPECT(tokens[15].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[16].is(TokenKind::kPlus));
  ZC_EXPECT(tokens[17].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[18].is(TokenKind::kSemicolon));
  ZC_EXPECT(tokens[19].is(TokenKind::kRightBrace));
  ZC_EXPECT(tokens[20].is(TokenKind::kEOF));
...
ZC_TEST("LexerTest.PeekNextToken") {
  ...
  ZC_EXPECT(!peeked.is(TokenKind::kEOF));
...
ZC_TEST("LexerTest.MultipleOperators") {
  auto tokens = tokenize("= += -= *= /= %= &= |= ^= <<= >>= <=> &&= \?\?="_zc);
  ZC_EXPECT(tokens.size() == 16);
  ZC_EXPECT(tokens[0].is(TokenKind::kEquals));
  ZC_EXPECT(tokens[1].is(TokenKind::kPlusEquals));
  ZC_EXPECT(tokens[2].is(TokenKind::kMinusEquals));
  ZC_EXPECT(tokens[3].is(TokenKind::kAsteriskEquals));
  ZC_EXPECT(tokens[4].is(TokenKind::kSlashEquals));
  ZC_EXPECT(tokens[5].is(TokenKind::kPercentEquals));
  ZC_EXPECT(tokens[6].is(TokenKind::kAmpersandEquals));
  ZC_EXPECT(tokens[7].is(TokenKind::kBarEquals));
  ZC_EXPECT(tokens[8].is(TokenKind::kCaretEquals));
  ZC_EXPECT(tokens[9].is(TokenKind::kLessThanLessThanEquals));
  ZC_EXPECT(tokens[10].is(TokenKind::kGreaterThanGreaterThanEquals));
  ZC_EXPECT(tokens[11].is(TokenKind::kLessThanEquals));
  ZC_EXPECT(tokens[12].is(TokenKind::kGreaterThan));
  ZC_EXPECT(tokens[13].is(TokenKind::kAmpersandAmpersandEquals));
  ZC_EXPECT(tokens[14].is(TokenKind::kQuestionQuestionEquals));
  ZC_EXPECT(tokens[15].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.BracesAndParentheses") {
  ...
  ZC_EXPECT(tokens[0].is(TokenKind::kLeftBrace));
  ZC_EXPECT(tokens[1].is(TokenKind::kRightBrace));
  ZC_EXPECT(tokens[2].is(TokenKind::kLeftParen));
  ZC_EXPECT(tokens[3].is(TokenKind::kRightParen));
  ZC_EXPECT(tokens[4].is(TokenKind::kLeftBracket));
  ZC_EXPECT(tokens[5].is(TokenKind::kRightBracket));
  ZC_EXPECT(tokens[6].is(TokenKind::kEOF));
}

ZC_TEST("LexerTest.MoreKeywords") {
  auto tokens = tokenize("let var const class interface implements extends"_zc);
  ZC_EXPECT(tokens.size() == 8);
  ZC_EXPECT(tokens[0].is(TokenKind::kLetKeyword));
  ZC_EXPECT(tokens[1].is(TokenKind::kVarKeyword));
  ZC_EXPECT(tokens[2].is(TokenKind::kConstKeyword));
  ZC_EXPECT(tokens[3].is(TokenKind::kClassKeyword));
  ZC_EXPECT(tokens[4].is(TokenKind::kInterfaceKeyword));
  ZC_EXPECT(tokens[5].is(TokenKind::kImplementsKeyword));
  ZC_EXPECT(tokens[6].is(TokenKind::kExtendsKeyword));
  ZC_EXPECT(tokens[7].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.ControlFlowKeywords") {
  ...
  ZC_EXPECT(tokens.size() == 7);
  ZC_EXPECT(tokens[0].is(TokenKind::kReturnKeyword));
  ZC_EXPECT(tokens[1].is(TokenKind::kBreakKeyword));
  ZC_EXPECT(tokens[2].is(TokenKind::kContinueKeyword));
  ZC_EXPECT(tokens[3].is(TokenKind::kMatchKeyword));
  ZC_EXPECT(tokens[4].is(TokenKind::kCaseKeyword));
  ZC_EXPECT(tokens[5].is(TokenKind::kDefaultKeyword));
  ZC_EXPECT(tokens[6].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.NumericLiteralsWithSuffixes") {
  ...
  for (size_t i = 0; i < tokens.size() - 1; ++i) {
    ZC_EXPECT(tokens[i].is(TokenKind::kIntegerLiteral) || tokens[i].is(TokenKind::kFloatLiteral));
  }
...
ZC_TEST("LexerTest.CharacterLiterals") {
  ...
  ZC_EXPECT(tokens.size() == 7);
  for (size_t i = 0; i < tokens.size() - 1; ++i) {
    ZC_EXPECT(tokens[i].is(TokenKind::kCharacterLiteral));
  }
  ZC_EXPECT(tokens[tokens.size() - 1].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.NestedComments") {
  ...
  ZC_EXPECT(tokens[2].is(TokenKind::kAsterisk));
  ZC_EXPECT(tokens[3].is(TokenKind::kSlash));
  ZC_EXPECT(tokens[4].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[5].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.CommentAttachToNextToken") {
  ...
  Token token;
  lexer.lex(token);
  ZC_EXPECT(token.is(TokenKind::kIdentifier));
}
...
ZC_TEST("LexerTest.SlashEqualsOperator") {
  ...
  ZC_EXPECT(tokens.size() == 4);
  ZC_EXPECT(tokens[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[1].is(TokenKind::kSlashEquals));
  ZC_EXPECT(tokens[2].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[3].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.ErrorPropagateAndDefaultOperators") {
  ...
  ZC_EXPECT(tokens.size() == 6);
  ZC_EXPECT(tokens[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[1].is(TokenKind::kErrorPropagate));
  ZC_EXPECT(tokens[2].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[3].is(TokenKind::kErrorDefault));
  ZC_EXPECT(tokens[4].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[5].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.DotDotDotOperator") {
  ...
  ZC_EXPECT(tokens.size() == 6);
  ZC_EXPECT(tokens[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[1].is(TokenKind::kLeftParen));
  ZC_EXPECT(tokens[2].is(TokenKind::kDotDotDot));
  ZC_EXPECT(tokens[3].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[4].is(TokenKind::kRightParen));
  ZC_EXPECT(tokens[5].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.UnicodeIdentifiers") {
  ...
  ZC_EXPECT(tokens.size() == 4);
  for (size_t i = 0; i < tokens.size() - 1; ++i) {
    ZC_EXPECT(tokens[i].is(TokenKind::kIdentifier));
  }
  ZC_EXPECT(tokens[tokens.size() - 1].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.ErrorRecovery") {
  ...
  ZC_EXPECT(tokens.size() == 5);
  ZC_EXPECT(tokens[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[1].is(TokenKind::kAt));
  ZC_EXPECT(tokens[2].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[3].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[4].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.LongIdentifiers") {
  ...
  ZC_EXPECT(tokens.size() == 2);
  ZC_EXPECT(tokens[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[1].is(TokenKind::kEOF));
}

ZC_TEST("LexerTest.EmptyInput") {
  auto tokens = tokenize(""_zc);
  ZC_EXPECT(tokens.size() == 1);
  ZC_EXPECT(tokens[0].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.MixedContent") {
  ...
  ZC_EXPECT(tokens[2].is(TokenKind::kLeftParen));
  ZC_EXPECT(tokens[3].is(TokenKind::kRightParen));
  ZC_EXPECT(tokens[4].is(TokenKind::kLeftBrace));
  ZC_EXPECT(tokens[5].is(TokenKind::kLetKeyword));
  ZC_EXPECT(tokens[6].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[7].is(TokenKind::kEquals));
  ZC_EXPECT(tokens[8].is(TokenKind::kIntegerLiteral));
  ZC_EXPECT(tokens[9].is(TokenKind::kSemicolon));
  ZC_EXPECT(tokens[10].is(TokenKind::kRightBrace));
  ZC_EXPECT(tokens[11].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.MultipleTokensLocation") {
  ...
  ZC_EXPECT(token1.is(TokenKind::kIdentifier));
  ZC_EXPECT(token2.is(TokenKind::kIdentifier));
  ZC_EXPECT(token3.is(TokenKind::kIdentifier));
...
ZC_TEST("LexerTest.LookAheadMultiple") {
  ...
  ZC_EXPECT(ahead1.is(TokenKind::kIdentifier));
  ZC_EXPECT(ahead2.is(TokenKind::kIdentifier));
  ZC_EXPECT(ahead3.is(TokenKind::kIdentifier));
}
...
ZC_TEST("LexerTest.ComplexOperators") {
  ...
  auto tokens = tokenize("=== !== <<= >>= >>>= **= \?\?="_zc);
  ZC_EXPECT(tokens.size() == 8);
  ZC_EXPECT(tokens[0].is(TokenKind::kEqualsEqualsEquals));
  ZC_EXPECT(tokens[1].is(TokenKind::kExclamationEqualsEquals));
  ZC_EXPECT(tokens[2].is(TokenKind::kLessThanLessThanEquals));
  ZC_EXPECT(tokens[3].is(TokenKind::kGreaterThanGreaterThanEquals));
  ZC_EXPECT(tokens[4].is(TokenKind::kGreaterThanGreaterThanGreaterThanEquals));
  ZC_EXPECT(tokens[5].is(TokenKind::kAsteriskAsteriskEquals));
  ZC_EXPECT(tokens[6].is(TokenKind::kQuestionQuestionEquals));
  ZC_EXPECT(tokens[tokens.size() - 1].is(TokenKind::kEOF));

  // Test arrow operator and question dot
  auto tokens2 = tokenize("-> ?. ... **"_zc);
  ZC_EXPECT(tokens2.size() == 5);
  ZC_EXPECT(tokens2[0].is(TokenKind::kArrow));
  ZC_EXPECT(tokens2[1].is(TokenKind::kQuestionDot));
  ZC_EXPECT(tokens2[2].is(TokenKind::kDotDotDot));
  ZC_EXPECT(tokens2[3].is(TokenKind::kAsteriskAsterisk));
  ZC_EXPECT(tokens2[tokens2.size() - 1].is(TokenKind::kEOF));

  // Test shift operators
  auto tokens3 = tokenize("<< >> >>> &= |= ^="_zc);
  ZC_EXPECT(tokens3.size() == 7);
  ZC_EXPECT(tokens3[0].is(TokenKind::kLessThanLessThan));
  ZC_EXPECT(tokens3[1].is(TokenKind::kGreaterThanGreaterThan));
  ZC_EXPECT(tokens3[2].is(TokenKind::kGreaterThanGreaterThanGreaterThan));
  ZC_EXPECT(tokens3[3].is(TokenKind::kAmpersandEquals));
  ZC_EXPECT(tokens3[4].is(TokenKind::kBarEquals));
  ZC_EXPECT(tokens3[5].is(TokenKind::kCaretEquals));
  ZC_EXPECT(tokens3[tokens3.size() - 1].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.UnicodeScalarValues") {
  ...
  Token token;
  lexer.lex(token);
  ZC_EXPECT(!token.is(TokenKind::kEOF));
...
ZC_TEST("LexerTest.EscapedIdentifierEdgeCases") {
  ...
  ZC_EXPECT(tokens2[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens2[tokens2.size() - 1].is(TokenKind::kEOF));

  // Test mixed escaped and regular characters
  auto tokens3 = tokenize("\\u0041bc\\u0044ef"_zc);
  ZC_EXPECT(tokens3.size() == 2);
  ZC_EXPECT(tokens3[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens3[tokens3.size() - 1].is(TokenKind::kEOF));

  // Test escaped identifier followed by keyword
  auto tokens4 = tokenize("\\u0041bc fun"_zc);
  ZC_EXPECT(tokens4.size() == 3);
  ZC_EXPECT(tokens4[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens4[1].is(TokenKind::kFunKeyword));
  ZC_EXPECT(tokens4[tokens4.size() - 1].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.OperatorPrecedenceAndAssociativity") {
  ...
  ZC_EXPECT(tokens.size() == 12);
  ZC_EXPECT(tokens[0].is(TokenKind::kPlusPlus));
  ZC_EXPECT(tokens[1].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[2].is(TokenKind::kMinusMinus));
  ZC_EXPECT(tokens[3].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[4].is(TokenKind::kAsteriskAsterisk));
  ZC_EXPECT(tokens[5].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[6].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[7].is(TokenKind::kQuestionQuestion));
  ZC_EXPECT(tokens[8].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[9].is(TokenKind::kQuestionDot));
  ZC_EXPECT(tokens[10].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens[11].is(TokenKind::kEOF));

  // Test assignment operators
  auto tokens2 = tokenize("x += y -= z *= w /= v %= u"_zc);
  ZC_EXPECT(tokens2.size() == 12);
  ZC_EXPECT(tokens2[1].is(TokenKind::kPlusEquals));
  ZC_EXPECT(tokens2[3].is(TokenKind::kMinusEquals));
  ZC_EXPECT(tokens2[5].is(TokenKind::kAsteriskEquals));
  ZC_EXPECT(tokens2[7].is(TokenKind::kSlashEquals));
  ZC_EXPECT(tokens2[9].is(TokenKind::kPercentEquals));
  ZC_EXPECT(tokens2[tokens2.size() - 1].is(TokenKind::kEOF));

  // Test bitwise operators
  auto tokens3 = tokenize("x & y | z ^ w ~ v"_zc);
  ZC_EXPECT(tokens3.size() == 10);
  ZC_EXPECT(tokens3[1].is(TokenKind::kAmpersand));
  ZC_EXPECT(tokens3[3].is(TokenKind::kBar));
  ZC_EXPECT(tokens3[5].is(TokenKind::kCaret));
  ZC_EXPECT(tokens3[tokens3.size() - 1].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.LexEscapedIdentifier") {
  ...
  ZC_EXPECT(tokens1[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens1[1].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens1[2].is(TokenKind::kEOF));

  // Test mixed escaped and normal characters
  auto tokens2 = tokenize("\\u0041bc\\u{44}ef"_zc);
  ZC_EXPECT(tokens2.size() == 2);
  ZC_EXPECT(tokens2[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens2[1].is(TokenKind::kEOF));

  // Test invalid escape sequences (should handle gracefully)
  auto tokens3 = tokenize("\\u123 \\u{} valid"_zc);
  ZC_EXPECT(tokens3.size() == 6);
  ZC_EXPECT(tokens3[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens3[1].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens3[2].is(TokenKind::kLeftBrace));
  ZC_EXPECT(tokens3[3].is(TokenKind::kRightBrace));
  ZC_EXPECT(tokens3[4].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens3[5].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.LexOperator") {
  ...
  ZC_EXPECT(tokens[2].is(TokenKind::kAsterisk));
  ZC_EXPECT(tokens[3].is(TokenKind::kSlash));
  ZC_EXPECT(tokens[4].is(TokenKind::kEquals));
  ZC_EXPECT(tokens[5].is(TokenKind::kLessThan));
  ZC_EXPECT(tokens[6].is(TokenKind::kGreaterThan));
  ZC_EXPECT(tokens[7].is(TokenKind::kExclamation));
  ZC_EXPECT(tokens[8].is(TokenKind::kAmpersand));
  ZC_EXPECT(tokens[9].is(TokenKind::kBar));
  ZC_EXPECT(tokens[10].is(TokenKind::kEOF));

  // Test multi-character operators
  auto tokens2 = tokenize("== != <= >= === !== <<= >>= >>>="_zc);
  ZC_EXPECT(tokens2.size() == 10);
  ZC_EXPECT(tokens2[0].is(TokenKind::kEqualsEquals));
  ZC_EXPECT(tokens2[1].is(TokenKind::kExclamationEquals));
  ZC_EXPECT(tokens2[2].is(TokenKind::kLessThanEquals));
  ZC_EXPECT(tokens2[3].is(TokenKind::kGreaterThanEquals));
  ZC_EXPECT(tokens2[4].is(TokenKind::kEqualsEqualsEquals));
  ZC_EXPECT(tokens2[5].is(TokenKind::kExclamationEqualsEquals));
  ZC_EXPECT(tokens2[6].is(TokenKind::kLessThanLessThanEquals));
  ZC_EXPECT(tokens2[7].is(TokenKind::kGreaterThanGreaterThanEquals));
  ZC_EXPECT(tokens2[8].is(TokenKind::kGreaterThanGreaterThanGreaterThanEquals));
  ZC_EXPECT(tokens2[9].is(TokenKind::kEOF));

  // Test arrow operators
  auto tokens3 = tokenize("=> ->"_zc);
  ZC_EXPECT(tokens3.size() == 3);
  ZC_EXPECT(tokens3[0].is(TokenKind::kEqualsGreaterThan));
  ZC_EXPECT(tokens3[1].is(TokenKind::kArrow));
  ZC_EXPECT(tokens3[2].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.LexUnicodeScalarValue") {
  ...
  Token token2;
  lexer2.lex(token2);
  ZC_EXPECT(!token2.is(TokenKind::kEOF));
  ...
  Token token3;
  lexer3.lex(token3);
  ZC_EXPECT(!token3.is(TokenKind::kEOF));
...
ZC_TEST("LexerTest.UnicodeInStringLiterals") {
  ...
  ZC_EXPECT(tokens2[0].is(TokenKind::kCharacterLiteral));
  ZC_EXPECT(tokens2[1].is(TokenKind::kCharacterLiteral));
  ZC_EXPECT(tokens2[2].is(TokenKind::kEOF));

  // Test mixed escape sequences in strings
  auto tokens3 = tokenize("\"Hello\\n\\u0041\\tWorld\\u{21}\"");
  ZC_EXPECT(tokens3.size() == 2);  // 1 string literal + EOF
  ZC_EXPECT(tokens3[0].is(TokenKind::kStringLiteral));
  ZC_EXPECT(tokens3[1].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.UnicodeDigitsInIdentifiers") {
  ...
  ZC_EXPECT(tokens2[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens2[1].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens2[2].is(TokenKind::kEOF));
  ...
  ZC_EXPECT(tokens4.size() == 3);
  ZC_EXPECT(tokens4[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens4[1].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens4[2].is(TokenKind::kEOF));

  // Test Thai digits (0x0e50-0x0e59) - ๐๑๒๓๔๕๖๗๘๙
  auto tokens5 = tokenize("id๐๑ var๒"_zc);
  ZC_EXPECT(tokens5.size() == 3);
  ZC_EXPECT(tokens5[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens5[1].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens5[2].is(TokenKind::kEOF));

  // Test Fullwidth digits (0xff10-0xff19) - ０１２３４５６７８９
  auto tokens6 = tokenize("id０１ var２"_zc);
  ZC_EXPECT(tokens6.size() == 3);
  ZC_EXPECT(tokens6[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens6[1].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens6[2].is(TokenKind::kEOF));

  // Test mixed Unicode digits in one identifier
  auto tokens7 = tokenize("mixed٠०০๐０"_zc);
  ZC_EXPECT(tokens7.size() == 2);
  ZC_EXPECT(tokens7[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens7[1].is(TokenKind::kEOF));
  ...
  ZC_EXPECT(tokens8[tokens8.size() - 1].is(TokenKind::kEOF));

  // Test Extended Arabic-Indic digits (0x06f0-0x06f9) - ۰۱۲۳۴۵۶۷۸۹
  auto tokens9 = tokenize("id۰۱۲ var۳"_zc);
  ZC_EXPECT(tokens9.size() == 3);
  ZC_EXPECT(tokens9[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens9[1].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens9[2].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.FourByteUTF8Sequences") {
  ...
  auto tokens2 = tokenize("\"🚀\""_zc);
  ZC_EXPECT(tokens2.size() == 2);
  ZC_EXPECT(tokens2[0].is(TokenKind::kStringLiteral));
  ZC_EXPECT(tokens2[1].is(TokenKind::kEOF));
...
ZC_TEST("LexerTest.ErrorPropagateOperator") {
  ...
  auto tokens = tokenize("?!"_zc);
  ZC_EXPECT(tokens.size() == 2);
  ZC_EXPECT(tokens[0].is(TokenKind::kErrorPropagate));
  ZC_EXPECT(tokens[1].is(TokenKind::kEOF));
  ...
  ZC_EXPECT(tokens2[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens2[1].is(TokenKind::kErrorPropagate));
  ZC_EXPECT(tokens2[2].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens2[3].is(TokenKind::kEOF));

  // Test mixed with other question mark operators
  auto tokens3 = tokenize("? ?! ?? ?:"_zc);
  ZC_EXPECT(tokens3.size() == 5);
  ZC_EXPECT(tokens3[0].is(TokenKind::kQuestion));
  ZC_EXPECT(tokens3[1].is(TokenKind::kErrorPropagate));
  ZC_EXPECT(tokens3[2].is(TokenKind::kQuestionQuestion));
  ZC_EXPECT(tokens3[3].is(TokenKind::kErrorDefault));
  ZC_EXPECT(tokens3[4].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.LogicalOrEqualsOperator") {
  ...
  auto tokens = tokenize("||="_zc);
  ZC_EXPECT(tokens.size() == 2);
  ZC_EXPECT(tokens[0].is(TokenKind::kBarBarEquals));
  ZC_EXPECT(tokens[1].is(TokenKind::kEOF));
  ...
  ZC_EXPECT(tokens2.size() == 4);
  ZC_EXPECT(tokens2[0].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens2[1].is(TokenKind::kBarBarEquals));
  ZC_EXPECT(tokens2[2].is(TokenKind::kIdentifier));
  ZC_EXPECT(tokens2[3].is(TokenKind::kEOF));
  ...
  auto tokens3 = tokenize("|| ||= && &="_zc);
  ZC_EXPECT(tokens3.size() == 5);
  ZC_EXPECT(tokens3[0].is(TokenKind::kBarBar));
  ZC_EXPECT(tokens3[1].is(TokenKind::kBarBarEquals));
  ZC_EXPECT(tokens3[2].is(TokenKind::kAmpersandAmpersand));
  ZC_EXPECT(tokens3[3].is(TokenKind::kAmpersandEquals));
  ZC_EXPECT(tokens3[4].is(TokenKind::kEOF));
  ...
  ZC_EXPECT(tokens4[0].is(TokenKind::kBar));
  ZC_EXPECT(tokens4[1].is(TokenKind::kBarEquals));
  ZC_EXPECT(tokens4[2].is(TokenKind::kBarBar));
  ZC_EXPECT(tokens4[3].is(TokenKind::kBarBarEquals));
  ZC_EXPECT(tokens4[4].is(TokenKind::kEOF));
}
...
ZC_TEST("LexerTest.FormTokenPathCoverage") {
  ...
  Token token1;
  lexer1.lex(token1);
  ZC_EXPECT(token1.is(TokenKind::kBacktick));  // Should form escaped identifier token
  ...
  Token token2;
  lexer2.lex(token2);
  ZC_EXPECT(token2.is(TokenKind::kBacktick));  // Should form Unicode escaped identifier
  lexer2.lex(token2);
  ZC_EXPECT(token2.is(TokenKind::kIdentifier));
  lexer2.lex(token2);
  ZC_EXPECT(token2.is(TokenKind::kBacktick));
  ...
  Token token3;
  lexer3.lex(token3);
  ZC_EXPECT(token3.is(TokenKind::kBacktick));
  lexer3.lex(token3);
  ZC_EXPECT(token3.is(TokenKind::kIdentifier));
  lexer3.lex(token3);
  ZC_EXPECT(token3.is(TokenKind::kIdentifier));
  ...
  Token token4;
  lexer4.lex(token4);
  ZC_EXPECT(token4.is(TokenKind::kBacktick));
  lexer4.lex(token4);
  ZC_EXPECT(token4.is(TokenKind::kIntegerLiteral));
  lexer4.lex(token4);
  ZC_EXPECT(token4.is(TokenKind::kIdentifier));
  lexer4.lex(token4);
  ZC_EXPECT(token4.is(TokenKind::kBacktick));
}
...
ZC_TEST("LexerTest.CommentRetentionMode") {
  ...
  Token token1;
  lexer1.lex(token1);
  ZC_EXPECT(token1.is(TokenKind::kIdentifier));  // Next token should be identifier
  ...
  Token token2;
  lexer2.lex(token2);
  ZC_EXPECT(token2.is(TokenKind::kIdentifier));  // Next token should be identifier
...
ZC_TEST("LexerTest.StateRestoreFunction") {
  ...
  ZC_EXPECT(lookahead1.is(TokenKind::kIdentifier));  // First token
  ZC_EXPECT(lookahead2.is(TokenKind::kIdentifier));  // Second token
  ...
  ZC_EXPECT(ahead1.is(TokenKind::kFloatLiteral));
  ZC_EXPECT(ahead2.is(TokenKind::kStringLiteral));
  ZC_EXPECT(ahead3.is(TokenKind::kIdentifier));

  // Test that normal lexing still works after lookahead
  Token current;
  lexer2.lex(current);
  ZC_EXPECT(current.is(TokenKind::kFloatLiteral));  // Should match lookahead
}
...
ZC_TEST("LexerTest.CacheTokenRangeEndPointer") {
  ...
  ZC_EXPECT(token17.is(TokenKind::kIdentifier));  // Should be token17
  ZC_EXPECT(token18.is(TokenKind::kIdentifier));  // Should be token18
  ...
  ZC_EXPECT(int_token.is(TokenKind::kIntegerLiteral));  // Should be 123
  ZC_EXPECT(str_token.is(TokenKind::kStringLiteral));   // Should be "str"
  ...
  ZC_EXPECT(current.is(TokenKind::kIntegerLiteral));  // Should be 123

  lexer2.lex(current);                               // Get second token
  ZC_EXPECT(current.is(TokenKind::kStringLiteral));  // Should be "str"

  lexer2.lex(current);                            // Get third token
  ZC_EXPECT(current.is(TokenKind::kIdentifier));  // Should be id1

  lexer2.lex(current);                              // Get fourth token
  ZC_EXPECT(current.is(TokenKind::kFloatLiteral));  // Should be 456.78

  lexer2.lex(current);                            // Get fifth token
  ZC_EXPECT(current.is(TokenKind::kIdentifier));  // Should be id2
}
