...
bool containsOccurrence(const StableImplementationOccurrenceHeaderProductionInput& input) {
  size_t matches = 0;
...
bool completeProjections(const StableImplementationOccurrenceHeaderProductionInput& input,
...
zc::Maybe<const identity::DefinitionKey&> definitionKeyAt(
    const StableImplementationOccurrenceHeaderProductionInput& input, ast::NodeId node) {
  for (const auto& site : input.definitionSites.entries()) {
...
bool matchesOwners(const StableImplementationOccurrenceHeaderProductionInput& input,
...
StableImplementationOccurrenceHeaderProducer::produce(
    const StableImplementationOccurrenceHeaderProductionInput& input) {
