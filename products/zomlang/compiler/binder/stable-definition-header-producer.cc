...
bool selectAuthority(const StableDefinitionHeaderProductionInput& input) {
...
bool completeProjections(const StableDefinitionHeaderProductionInput& input,
                         ast::NodeId moduleNode) {
  auto reconstructed = StableIdentityCandidateVerifier::reconstruct(
...
zc::Maybe<const identity::DefinitionKey&> definitionKeyAt(
    const StableDefinitionHeaderProductionInput& input, ast::NodeId node) {
  for (const auto& site : input.definitionSites.entries()) {
...
bool matchesOwners(const StableDefinitionHeaderProductionInput& input,
...
zc::Maybe<CanonicalSequence<StableHeaderGenericParameter>> buildGenericParameters(
    const StableDefinitionHeaderProductionInput& input, const DefinitionInventory& inventory,
    ast::NodeId owner) {
...
zc::Maybe<StableDefinitionHeader> StableDefinitionHeaderProducer::produce(
    const StableDefinitionHeaderProductionInput& input) {
