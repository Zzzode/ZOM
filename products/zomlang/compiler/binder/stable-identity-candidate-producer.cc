...
public:
  StableIdentityCandidateProducerImpl(const CanonicalParsedModule& parsedModule,
...
private:
  ...
  bool selectedModule(ast::NodeId candidateModule) const noexcept {
    return candidateModule == moduleNode || (!candidateModule && !moduleNode);
  }
  ...
  void collectReferences() {
    for (size_t index = 0; index < inventory.definitions().size(); ++index) {
      ...
      if (selectedModule(definition.moduleNode) &&
          stableOwnerChain(definition.parentPath.asPtr())) {
    ...
    for (size_t index = 0; index < inventory.impls().size(); ++index) {
      ...
      if (selectedModule(implementation.moduleNode) &&
  ...
  zc::Maybe<const DefinitionInventoryEntry&> definition(ast::NodeId node) const {
    for (const auto& entry : inventory.definitions()) {
      if (entry.node == node && selectedModule(entry.moduleNode)) { return entry; }
    }
    return zc::none;
  }
...
StableIdentityCandidateProduction StableIdentityCandidateProducer::produce(
    const CanonicalParsedModule& parsedModule, const identity::ModuleKey& module,
    ast::NodeId moduleNode) {
  return StableIdentityCandidateProducerImpl(parsedModule, module, moduleNode).produce();
}
