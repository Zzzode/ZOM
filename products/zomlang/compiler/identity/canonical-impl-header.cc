...
CanonicalImplHeader::CanonicalImplHeader(
    zc::Own<canonical_impl_header_detail::CanonicalImplHeaderData>&& value) noexcept
    : impl(zc::mv(value)) {}

CanonicalImplHeader::~CanonicalImplHeader() noexcept(false) = default;
CanonicalImplHeader::CanonicalImplHeader(CanonicalImplHeader&&) noexcept = default;
CanonicalImplHeader& CanonicalImplHeader::operator=(CanonicalImplHeader&&) noexcept = default;

zc::Maybe<CanonicalImplHeader> CanonicalImplHeader::from(
  ...
  return CanonicalImplHeader(zc::heap<canonical_impl_header_detail::CanonicalImplHeaderData>(
...
zc::ArrayPtr<const CanonicalGenericParameter> CanonicalImplHeader::genericParameters()
    const noexcept {
  return impl->genericParameters.asPtr();
}

ImplPolarity CanonicalImplHeader::polarity() const noexcept { return impl->polarity; }

ImplSafety CanonicalImplHeader::safety() const noexcept { return impl->safety; }

const CanonicalTraitReference& CanonicalImplHeader::trait() const noexcept { return impl->trait; }

const CanonicalHeaderTypeSyntax& CanonicalImplHeader::selfType() const noexcept {
  return impl->selfType;
}

zc::ArrayPtr<const CanonicalBoundObligation> CanonicalImplHeader::obligations() const noexcept {
  return impl->obligations.asPtr();
}

void CanonicalImplHeader::encode(CanonicalEncoder& encoder) const {
...
zc::Array<uint8_t> CanonicalImplHeader::encode() const {
  CanonicalEncoder encoder;
  encode(encoder);
  return encoder.finish();
}
