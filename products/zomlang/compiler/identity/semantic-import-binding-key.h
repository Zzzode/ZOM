...
public:
  SemanticImportBindingKey(SemanticImportBindingKey&&) noexcept = default;
  SemanticImportBindingKey& operator=(SemanticImportBindingKey&&) noexcept = default;
  ZC_DISALLOW_COPY(SemanticImportBindingKey);

  /// \brief Admits exactly one semantic namespace-and-name import slot.
  ZC_NODISCARD static zc::Maybe<SemanticImportBindingKey> from(
  ...
  ZC_NODISCARD SemanticImportBindingKey clone() const;
  ...
  bool operator==(const SemanticImportBindingKey& other) const;
  bool operator!=(const SemanticImportBindingKey& other) const { return !(*this == other); }

private:
  SemanticImportBindingKey(ModuleKey&& requester, ModuleResolutionKey&& resolution,
