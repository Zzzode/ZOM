...
Ed25519PublicKey::Ed25519PublicKey(zc::Array<zc::byte>&& bytes) noexcept
    : value(zc::mv(bytes)) {}

zc::Maybe<Ed25519PublicKey> Ed25519PublicKey::fromBytes(
...
Ed25519Signature::Ed25519Signature(zc::Array<zc::byte>&& bytes) noexcept
    : value(zc::mv(bytes)) {}

zc::Maybe<Ed25519Signature> Ed25519Signature::fromBytes(
    zc::ArrayPtr<const zc::byte> bytes) {
...
identity::Sha256Digest SodiumRuntime::hashSha256(zc::ArrayPtr<const zc::byte> input) const {
  ...
  const int status = crypto_hash_sha256(digestBytes, input.begin(), input.size());
  ZC_IREQUIRE(status == 0, "libsodium SHA-256 failed");
