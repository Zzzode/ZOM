...
An ordinary impl publishes one impl head consisting of one interface
instantiation and one self-type pattern. A bodyless marker declaration
publishes one concrete marker fact keyed by marker identity and closed target
type. Transparent aliases do not create a new nominal declaration for the
orphan rule.

An ordinary impl with a bare generic self type is a blanket impl. Marker facts
are concrete and cannot contain impl-owned or unresolved type parameters.
Other compound generic overlap and constraint satisfiability are outside the
implemented coherence surface.
...
Two ordinary impls conflict when they name the same interface and either:

- their self-type names are equal; or
- either impl is a blanket impl.

Two explicit marker facts conflict when they have the same marker identity and
the same closed target type. Positive and negative facts for the same key also
conflict.
...
```mermaid
flowchart TD
    I[Ordinary impl or marker fact] --> L{Interface, marker, or self type local?}
    L -->|no| O[ZOM4054 OrphanImpl]
    L -->|yes| H[Compare canonical impl heads or marker keys]
...
- duplicate marker facts; and
- conflicting positive and negative marker facts for one key.
