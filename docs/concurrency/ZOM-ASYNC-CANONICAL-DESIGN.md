...
| **CR-4 Marker / OOP Split** | Three interface categories: regular OOP interface (class implements explicitly), marker interface (empty body + structural field recursion), unsafe marker (explicit negative-impl whitelist declaration). The three categories MUST NOT be mixed into the same interface. |
