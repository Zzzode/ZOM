...
| 2026-09-19 | DRAFT | Slice 4 implemented: `zomc emitBinary` runs the structural verifier and the multi-function translation validator on the presented MIR set between lowering and LLVM translation, aborting the emission transaction with an internal error on a finding. The `object-emission-cli` (seven shapes including zero/one/two-argument calls and the three-function leaf module) and `native-run-cli` (real host execution, exit 42) ctests cross the gate end to end. |
