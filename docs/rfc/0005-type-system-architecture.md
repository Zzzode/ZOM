...
|---|---|---|---|
| 1 | Complete | `type-test.cc`, `type-interner-test.cc`, and `type-env-test.cc` cover all concrete type classes and canonical IDs, including interface, existential, and associated forms. Type variables are ID-bearing values created through `TypeEnv` and use union-find/id bindings rather than name-keyed fallback bindings. `type-algebra.cc` centralizes structural type cloning, and `TypeAlgebra.ClonePreservesCompositeStructure` covers function, generic, raises, reference, union, intersection, existential, and associated-type cloning. `TypeEnv.InstantiatePreservesMonomorphicExistential`, `TypeEnv.InstantiatePreservesMonomorphicAssociatedType`, and `TypeEnv.InstantiatePreservesMonomorphicIntersection` cover composite substitution fidelity, while `TypeEnv.GeneralizeCollectsExistentialInterfaceTypeVars`, `TypeEnv.GeneralizeCollectsAssociatedParentTypeVars`, and `TypeEnv.GeneralizeCollectsIntersectionConjunctTypeVars` cover composite free-type-variable traversal. | None. |
...
| 3 | Complete | `coercion-test.cc` covers never, any, reference reborrow, raw-pointer mut-to-const, union injection, nullable union, rejection, and dyn upcast. `body-checker-test.cc` covers coercion records for function arguments, return statements, assignments, conditional joins, struct literal fields, nullable local initializers, and explicit existential erasure at annotated local sites. `union_injection_coercions_pos_18.check` covers user-visible union member injection at local initializer, function call argument, return, assignment, and struct field sites. | None. |
| 4 | Complete | `decl-signature-test.cc` covers function, class, interface, enum, alias, variable, const declaration, generic parameter, shared generic type variable, type expression, raises, symbol-keyed parameter/field signatures, recursive aliases, unsupported internal type-expression fallback diagnostics, and `GenericParams.where_` bounds feeding generic upper bounds. `DeclSignatureComputer`, `TraitResolver`, and `BodyChecker` resolve reference and raw-pointer type expressions from the schema-owned `ReferenceTypeExpr` and `RawPointerTypeExpr` nodes directly, with no checker-local compatibility path for older unary-expression test encodings. | None for current function-level where-bound signature computation. |
| 5 | Complete | `body-checker-test.cc` covers literals, identifiers, binary/unary/postfix operators, calls, returns, assignment, if/while/for, conditionals, nested blocks, arrays, tuples, object literals, struct literals with unknown/missing field rejection and field coercions, member access, index, casts, unsafe blocks, `is`, `this`, nullable coalesce, lambdas and function expressions with annotated body checks, match statement integration, error operators, and checker-internal fallback diagnostics for unresolved values, empty error unions, and malformed struct-literal targets. `call_non_function_neg_09.check` covers user-visible ZOM4012 through the diagnostics conformance runner for calling a non-function value, `binary_invalid_operands_neg_32.check` covers user-visible ZOM4028 for invalid arithmetic operands, `comparison_invalid_operands_neg_33.check` covers user-visible ZOM4029 for invalid comparison operands, `deref_non_pointer_neg_26.check` covers user-visible ZOM4030 for dereferencing a non-pointer value, `postfix_update_non_numeric_neg_34.check` covers user-visible ZOM4031 for postfix update on a non-numeric operand, `call_arg_count_neg_20.check` covers user-visible ZOM4036 for function call argument-count mismatch, `index_array_non_integer_neg_22.check` covers user-visible ZOM4038 for a non-integer array index, `index_tuple_non_literal_neg_23.check` covers user-visible ZOM4039 for a non-literal tuple index, `index_tuple_oob_neg_24.check` covers user-visible ZOM4040 for tuple index bounds, `index_non_indexable_neg_21.check` covers user-visible ZOM4041 for indexing a non-array/non-tuple value, `ternary_condition_non_bool_neg_25.check` covers user-visible ZOM4045 for a non-bool ternary condition, `return_missing_value_neg_07.check` covers user-visible ZOM4046 for `return;` in a non-unit function, `struct_literal_unknown_field_neg_27.check` covers user-visible ZOM4048 for unknown struct literal fields, `struct_literal_missing_field_neg_28.check` covers user-visible ZOM4049 for missing required struct literal fields, `array_literal_element_mismatch_neg_29.check` covers user-visible ZOM4050 for incompatible array literal element types, and `match_guard_non_bool_neg_08.check` covers user-visible ZOM4051 for a non-bool match guard. | None. |
...
| 7 | Complete | `body-checker-test.cc` covers explicit type argument substitution for `identity<f64>(42.0)`, wrong explicit type-argument count rejection with stable ZOM4016 diagnostics, explicit type arguments on non-generic callees with stable ZOM4035 diagnostics, and incompatible explicit type argument rejection. `generic_call_relational_disambig_pos_01.check` covers parsed call type arguments without confusing relational operators. `generic_explicit_success_pos_12.check` covers the positive explicit type argument path through the CLI pipeline. `generic_explicit_count_neg_10.check` covers the wrong explicit type-argument count through the diagnostics conformance runner with normal source/caret display, `generic_explicit_non_generic_neg_13.check` covers explicit type arguments on a non-generic callee through user-visible ZOM4035 source/caret output, `generic_explicit_incompatible_neg_11.check` covers explicit type arguments that conflict with call arguments through user-visible ZOM4009 source/caret output, and `generic_explicit_unsupported_type_arg_neg_14.check` covers user-visible ZOM4034 for an unsupported explicit function-type argument while proving the ANTLR grammar accepts full `TypeExpression` type arguments. | None. |
| 8 | Complete | `body-checker-test.cc` covers call-site rejection for an unsatisfied function-level interface bound, positive satisfaction through an impl block, and checker diagnostic ZOM4018 through `CheckerTraitNotImplemented`. `generic_bound_missing_neg_01.check` covers the same unsatisfied generic bound through the diagnostics conformance runner, and `generic_bound_satisfied_pos_10.check` covers the positive satisfied-bound CLI path. `DeclSignature.FunctionGenericParamPreservesWhereBound` covers where-clause bounds entering function signatures. `where_clause_pos_01.check`, `where_clause_pos_02.check`, `where_clause_pos_06.check`, `class_where_clause_pos_17.check`, and `impl_where_clause_pos_13.check` cover function, struct, class, and standalone impl where-clause AST retention. `docs/spec/chapters/12-generics.md`, `docs/spec/chapters/09-interfaces.md`, and `docs/spec/chapters/17-grammar-reference.md` state that interface declarations do not accept `where` clauses; `iface_where_reject_neg_05.check` covers that intentional parser-level rejection, so interface type-parameter constraints remain in the type-parameter list. `complex_impl_pos_09.check` covers parser/lit impl syntax with generic interface arguments. | Impl-level where-bound solving remains part of the broader trait resolver contract. |
| 9 | Complete | `trait-resolver-test.cc` covers duplicate concrete impl coherence, direct-vs-blanket overlap, and cross-module orphan-impl fallback diagnostics, with stable ZOM4017 (`ConflictingImpl`) and ZOM4054 (`OrphanImpl`) assertions. `conflicting_impl_neg_17.check` covers the duplicate-impl coherence error through the diagnostics conformance runner with normal source/caret display. | None. |
...
| 12 | Complete | `exhaustiveness-test.cc` covers booleans, open types, unions, wildcard, redundancy, constructors, and wildcard-pass cases. `BodyChecker.MatchStmtReportsNonExhaustiveEnum` covers enum integration through `BodyChecker::checkMatchStmt` with non-exhaustive diagnostics, and `BodyChecker.MatchStmtAcceptsExhaustiveTupleEnum` covers tuple enum variants with payload bindings. `match_exhaustive_bool_pos_02.check` and `match_non_exhaustive_bool_neg_01.check` cover user-visible bool match acceptance/rejection. `match_exhaustive_enum_pos_06.check` and `match_non_exhaustive_enum_neg_05.check` cover user-visible tuple enum match acceptance and ZOM4022 source-caret output for a missing variant. | None. |
...
| 15 | Complete | `checker-test.cc` and `body-checker-test.cc` verify fail-closed error nodes for local init, calls, arrays, struct literals, casts, and anonymous functions. `BodyChecker.DependentErrorExpressionEmitsOnlyOneDiagnostic` verifies one source type error produces one diagnostic while dependent expressions become `ErrorType`. `dependent_error_array_neg_36.check` covers the same behavior through the diagnostics conformance runner and asserts that an invalid binary expression inside an array literal does not cascade into `ZOM4050`. | None. |
| 16 | Complete | `body-checker-test.cc` covers `?!` requiring matching `raises`, `raises A | B` subset acceptance, raising-call propagation, and dedicated checker diagnostic ZOM4025 through `ErrorPropagateOutsideRaises`. `error_propagate_outside_raises_neg_11.check` covers ZOM4025 through the diagnostics conformance runner with normal source/caret display, and `error_propagate_non_union_neg_14.check` covers user-visible ZOM4032 when `?!` is applied to a non-error-union operand. `error-handling-operators.check`, `return_error_prop_pos_08.check`, and `fun_raises_pipe_pos_01.check` cover parser/lit syntax for `?!` and raises unions. | None. |
...
| 18 | Complete | `trait-resolver-test.cc` covers primitive marker derivation, structural object rejection through raw pointer fields, named struct positive/negative field-based derivation, explicit negative marker impl suppression, parser-shaped `AttributePath` marker impl names, and explicit unsafe marker impl override. `raw_pointer_field_marker_missing_neg_15.check` covers user-visible `ZOM4018` rejection for both `Sendable` and `Shared` bounds when a class contains a raw pointer field. `marker_impl_where_pos_14.check` and `marker_impl_short_neg_pos_16.check` cover user-visible `MarkerImpl` AST retention for qualified generic marker impls with `where` clauses and short negative marker impls. `qualified_impl_pos_15.check` proves qualified interface impls remain `StandaloneImplDecl` rather than being captured by marker-impl parsing. | None. |
...
| 20 | Complete | `query-cycle-detector-test.cc` covers signature, alias, associated-projection, and marker-derivation query keys. `decl-signature-test.cc` covers recursive aliases returning an error path, and `recursive_alias_cycle_neg_12.check` covers user-visible ZOM4052 through the diagnostics conformance runner. | None for implemented query users. |
...
- **Conformance:** Relevant conformance suites must still pass.
...
| 2026-07-08 | REVIEW | Added user-visible CLI conformance coverage for positive generic identity inference (`generic_infer_identity_pos_11`). |
| 2026-07-08 | REVIEW | Added user-visible CLI conformance coverage for positive user-defined `Add` operator trait dispatch (`operator_trait_add_pos_10`). |
| 2026-07-08 | REVIEW | Added user-visible CLI conformance coverage for exhaustive bool match acceptance (`match_exhaustive_bool_pos_02`). |
| 2026-07-08 | REVIEW | Added user-visible CLI conformance coverage for positive user-defined `Index` operator trait dispatch (`index_trait_pos_07`). |
| 2026-07-08 | REVIEW | Added user-visible CLI conformance coverage for positive user-defined `Eq` and `Ord` comparison operator trait dispatch (`eq_trait_pos_08`, `ord_trait_pos_09`). |
...
| 2026-07-08 | REVIEW | Added implementation evidence for `dyn I + marker` semantics, including marker-bound existential identity, interning, unification, concrete-to-dyn erasure, negative marker impl rejection, and parser-shaped `AttributePath` marker impl names. |
| 2026-07-08 | REVIEW | Added diagnostics conformance evidence for user-defined `Pow` operator trait dispatch and missing/signature-mismatched `Pow` implementations. |
...
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for function call argument-count mismatches (`ZOM4036`). |
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for indexing non-array/non-tuple values (`ZOM4041`). |
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for array and tuple index errors (`ZOM4038`, `ZOM4039`, and `ZOM4040`). |
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for non-bool ternary conditions (`ZOM4045`). |
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for dereferencing non-pointer values (`ZOM4030`). |
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for explicit type arguments on non-generic callees (`ZOM4035`). |
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for unknown and missing struct literal fields (`ZOM4048` and `ZOM4049`). |
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for missing return values and non-bool match guards (`ZOM4046` and `ZOM4051`). |
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for array literal element type mismatches (`ZOM4050`). |
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for non-error-union `?!`, raw-pointer unsafe cast gating, and invalid dyn upcasts (`ZOM4032`, `ZOM4043`, and `ZOM4044`). |
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for unsupported cast target types (`ZOM4042`). |
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for invalid binary operands, invalid comparison operands, and postfix update on non-numeric operands (`ZOM4028`, `ZOM4029`, and `ZOM4031`). |
| 2026-07-09 | REVIEW | Added diagnostics conformance coverage for recursive type alias cycles (`ZOM4052`). |
| 2026-07-09 | REVIEW | Aligned ANTLR generic call parsing with the handwritten parser for function-type explicit arguments and added diagnostics conformance coverage for unsupported explicit type arguments (`ZOM4034`). |
...
| 2026-07-09 | REVIEW | Removed obsolete checker compatibility paths that interpreted reference and raw-pointer type expressions from unary-expression test encodings; declaration signatures, body checks, and trait-resolution field checks now rely on the dedicated AST schema nodes. |
| 2026-07-09 | REVIEW | Added diagnostics conformance evidence that raw pointer fields prevent automatic `Sendable` and `Shared` marker derivation. |
| 2026-07-09 | REVIEW | Removed no-ID `TypeVar` construction and name-keyed type-variable binding fallback so inference variables are consistently owned and identified by `TypeEnv`. |
| 2026-07-09 | REVIEW | Added diagnostics conformance evidence that dependent expressions suppress cascading diagnostics after a source type error. |
| 2026-07-09 | REVIEW | Added positive conformance evidence for union member injection at local initializer, function call argument, and return sites. |
