...
| 11 | Complete | `type-test.cc` covers invariant `NamedType` generic arguments. `generic_invariant_named_type_neg_12.check` covers the user-visible CLI rejection of `Vec<dyn Drawable>` where `Vec<dyn Shape>` is expected, proving named generic arguments remain invariant even when the inner `dyn Drawable` can upcast to `dyn Shape`. | None for v1 invariance. |
