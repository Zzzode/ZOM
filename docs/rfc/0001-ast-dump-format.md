...
implementation: compiler/ast/dump.cc
...
- Decode nodes through `compiler/ast/schema.yml` metadata.
...
- Make AST conformance snapshots reviewable in `tests`.
...
| Area | Paths | Owner |
|---|---|---|
| AST schema and generated metadata | `compiler/ast/**`, `scripts/codegen/gen_ast.py` | `lexer-parser` |
| CLI dump command | `utils/zomc/**`, `compiler/basic/**` | `lexer-parser` |
| Conformance snapshots | `tests/conformance/**` | `verification` |
| Lit regeneration | `tests/tools/regen-lit.py` | `verification` |
...
The conformance test documentation and `tests/tools/regen-lit.py`
...
metadata in sync with `compiler/ast/schema.yml` and requiring
...
7. Update `tests/tools/regen-lit.py` to regenerate tree-format
   expectations.
