...
| Parsed declaration inventory | `compiler/lexer/**`, `compiler/parser/**`, `compiler/ast/**` | `lexer-parser` |
| Shared identity values, keys, encoders, registries, and invariant facts | `compiler/identity/**` | `module-system` |
| Context, package, crate, module, and source identity | `compiler/driver/**`, `compiler/symbol/**`, `compiler/source/**` | `module-system` |
| Definition, impl, and semantic type handle consumers | `compiler/binder/**`, `compiler/checker/**`, `compiler/type/**` | `binder-checker` |
| Identity invariant diagnostics | `compiler/diagnostics/**` | `error-system` |
| HIR, MIR, LIR, and backend identity consumers | `compiler/hir/**`, `compiler/mir/**`, `compiler/lir/**`, `compiler/backend/**` | `ir-backend` |
| Compiler build wiring for the identity library | `compiler/CMakeLists.txt` | `ir-backend` |
| Package, crate, module, and architecture alignment | `docs/spec/**`, `docs/design/**` | `spec-audit` |
| Identity, determinism, and cross-context tests | `tests/**` | `verification` |
...
2. Add `compiler/identity/**` and route it to the module-system
