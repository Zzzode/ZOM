...
|---|---|---|---|
| 6 | Partial | `products/zomlang/runtime/panic.h` and `panic.cc` define `ZomPanicInfo`, `ZomPanicKind`, `formatPanicInfo`, `__zom_panic`, `__zom_begin_panic_unwind`, `__zom_abort_panic`, and `__zom_catch_unwind`. `__zom_abort_panic` writes deterministic panic metadata to stderr before aborting through a best-effort no-allocation path. `panic-test.cc` verifies stable panic kind names, deterministic panic metadata formatting, that `__zom_catch_unwind` calls its thunk and returns `false` under the current abort strategy, that `__zom_abort_panic` raises `SIGABRT` in a subprocess, that abort stderr contains the formatted metadata line, and that exported panic ABI symbols link. | Add real unwind support or explicit unsupported-target rejection before accepting `panic = "unwind"`. |
