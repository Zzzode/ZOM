...
`zomlang/compiler/binder/stable-binding-schema.def` is the
...
`zomlang/compiler/driver/query/module-graph/module-graph-query-input.h` and
`zomlang/compiler/driver/query/module-graph/module-graph-query-input.cc`. I2A through
...
additive `zomlang/compiler/driver/CMakeLists.txt` rows for
...
`zomlang/compiler/query/query-database.h`, the
`MaterializeModuleGraph` production inventory row in
`zomlang/compiler/query/query-descriptor-schema.def`, and the
...
| 8 | routing governance | assign `zomlang/compiler/ownership/**` to `runtime-memory`; assign `scripts/check-english-only.py` and `scripts/check-spec-alignment.py` to `verification`; synchronize `.codex/subagents/manifest.yaml`, `.codex/subagents/README.md`, `.codex/subagents/task-router.md`, `.codex/subagents/module-system.md`, `.codex/subagents/runtime-memory.md`, `.codex/subagents/verification.md`, and `AGENTS.md`; delete the stale database-owned semantic-identity rule |
...
| Area | Paths | Owner |
|---|---|---|
| Routing and manifest ownership | `.codex/subagents/**`; `AGENTS.md` | `task-router` |
| RFC governance | `docs/rfc/**`; `.codex/skills/rfc/**` | `rfc` |
| Query, identity, source, module graph, session, and module interface | `zomlang/compiler/identity/**`; `zomlang/compiler/query/**`; `zomlang/compiler/source/**`; `zomlang/compiler/binder/module-*`; `zomlang/compiler/driver/**`; `core/Zom.toml` | `module-system` |
| Stable Binder facts, materializers, Checker, and semantic types | `zomlang/compiler/binder/**` excluding `binder/module-*`; `zomlang/compiler/checker/**` excluding `checker/checker-source-diagnostics.def`; `zomlang/compiler/type/**` | `binder-checker` |
| Diagnostic schemas and materialization | `zomlang/compiler/diagnostics/**`; `zomlang/compiler/checker/checker-source-diagnostics.def` | `error-system` |
| HIR, shared IR, MIR, LIR, backend, and build wiring | `zomlang/compiler/hir/**`; `zomlang/compiler/ir/**`; `zomlang/compiler/mir/**`; `zomlang/compiler/lir/**`; `zomlang/compiler/backend/**`; the `ir-backend` build paths listed in `.codex/subagents/manifest.yaml` | `ir-backend` |
| Arena, interner concurrency review, ownership analysis, runtime, and core source | `libraries/zc/**`; `zomlang/compiler/ownership/**`; non-concurrency `zomlang/runtime/**`; `core/README.md`; `core/src/**` | `runtime-memory` |
...
| Native tests, CI, gates, coverage, benchmarks, corpus, and baseline | only the `verification` paths enumerated in `.codex/subagents/manifest.yaml`, including `zomlang/tests/**`, `.github/workflows/**`, `scripts/check-english-only.py`, `scripts/check-spec-alignment.py`, the named architecture/codegen/coverage scripts, coverage inputs, benchmark corpus and baseline, and coverage CMake utilities | `verification` |

The acceptance transaction assigns `zomlang/compiler/ownership/**`
...
| `G2` | `task-router` | `G1` | `.codex/subagents/manifest.yaml`; `.codex/subagents/README.md`; `.codex/subagents/task-router.md`; `.codex/subagents/module-system.md`; `.codex/subagents/runtime-memory.md`; `.codex/subagents/verification.md`; `AGENTS.md` | assign ownership analysis to `runtime-memory`, assign `scripts/check-english-only.py` and `scripts/check-spec-alignment.py` to `verification`, and synchronize exact gate routing; RFC and English-only gates |
...
| `G4` | `verification` | `G3` | `zomlang/tests/coverage/implementation-series-base.txt` | from a clean committed accepted synchronization tree, record that exact commit SHA and reject a moving or non-ancestor base |
| `S1` | `binder-checker` with `verification` review | `G4`; RFC 0031 `R31-09` | `zomlang/compiler/binder/stable-binding-schema.def`; `scripts/check-stable-binding-schema.py` | prepare and review the hand-authored closed field, sum, tag, domain, bound, mutation, artifact-task, capability-payload, failure-alternative, and diagnostic-mapping inventory without an independent landing |
| `S2` | `binder-checker` for Binder facts; `module-system` for driver contextual declarations and callers; `verification` for native tests | `S1` review | `zomlang/compiler/binder/stable/stable-binding-facts.h`; `zomlang/compiler/binder/stable/stable-binding-facts.cc`; `zomlang/compiler/driver/query/binding/contextual-binding-key.h`; `zomlang/compiler/driver/query/binding/contextual-binding-key.cc`; the complete driver caller-cutover files and tests named by RFC 0030 | prepare and review stable facts plus driver-owned contextual keys and the direct deletion and migration of all query-specific declarations without an independent landing |
| `S3` | `binder-checker` for Binder codecs; `module-system` for contextual codecs; `verification` for tests, build discovery, schema, architecture, allowlist, and landing-scope gates | `S2` review | `zomlang/compiler/binder/stable/stable-binding-codec.h`; `zomlang/compiler/binder/stable/stable-binding-codec.cc`; the matching contextual codecs, Binder build and test wiring, schema and architecture gates, exact allowlist, and landing-scope gate named by RFC 0030 | prepare and review exact-consumption codecs, sequence admission, fixed wire oracles, native tests, mutations, build visibility, and landing-scope proof without an independent landing |
| `S3A` | `binder-checker` with `module-system` integration and `verification` review | `S3`; RFC 0029 `R29-14` | `zomlang/compiler/binder/identity/named-identity-inventory.h`; `zomlang/compiler/binder/identity/named-identity-inventory.cc`; `zomlang/compiler/binder/identity/revision-local-identity-sites.cc`; `zomlang/compiler/driver/query/binding/active-definition-authority-session.cc`; `zomlang/compiler/driver/query/binding/named-item-query.cc`; `zomlang/compiler/driver/session/compiler-session.cc`; `zomlang/tests/unittests/compiler/driver/active-definition-authority-session-test.cc`; `zomlang/tests/unittests/compiler/driver/incremental-binding-query-adapter-test.cc` | land typed complete definition and implementation inventory entries, the exact codecs and mutation coverage, and the direct production caller cutover |
| `S3B` | `module-system` with `binder-checker` and `verification` review | `S3A` | `zomlang/compiler/driver/query/binding/named-identity-inventory-query.h`; `zomlang/compiler/driver/query/binding/named-identity-inventory-query.cc`; `zomlang/tests/unittests/compiler/driver/named-identity-inventory-query-test.cc`; `scripts/check-binder-architecture.py` | read selected parse provenance, derive body disposition independently in provider and verifier, prove exact read ordering and failures, and enforce the architecture boundary |
| `S4` | `binder-checker` with `verification` review | `S3B` | `zomlang/compiler/binder/stable/definition/header-producer.h`; `zomlang/compiler/binder/stable/definition/header-producer.cc`; `zomlang/compiler/binder/CMakeLists.txt`; `zomlang/tests/unittests/compiler/binder/stable/definition/header-producer-test.cc`; `zomlang/tests/unittests/compiler/binder/CMakeLists.txt` | definition-header production from the closed borrowed input record, retained parse provenance, and complete syntax-role matrix |
| `S4A` | `binder-checker` with `verification` review | `S3B` | `zomlang/compiler/binder/stable/implementation/header-producer.h`; `zomlang/compiler/binder/stable/implementation/header-producer.cc`; `zomlang/compiler/binder/CMakeLists.txt`; `zomlang/tests/unittests/compiler/binder/stable/implementation/header-producer-test.cc`; `zomlang/tests/unittests/compiler/binder/CMakeLists.txt` | implementation-occurrence header production from the closed borrowed input record with complete identity, source-form, and scope-role coverage |
| `S5` | `binder-checker` with `verification` review | `S4`; `S4A` | `zomlang/compiler/binder/stable/header/verifier.h`; `zomlang/compiler/binder/stable/header/verifier.cc`; `zomlang/compiler/binder/stable-binding-schema.def`; `zomlang/compiler/binder/CMakeLists.txt`; `zomlang/tests/unittests/compiler/binder/stable/header/verifier-test.cc`; `zomlang/tests/unittests/compiler/binder/stable-binding-query-test.cc`; `zomlang/tests/unittests/compiler/binder/CMakeLists.txt`; `scripts/check-stable-binding-schema.py`; `scripts/check-binder-architecture.py` | independent full-context selection and header verification, exact schema provenance names, equal-occurrence coverage, caller-selected-entry rejection, and producer/verifier disagreement mutations |
...
| `Q3` | `module-system` | `G3` | `zomlang/compiler/driver/package/canonical-package-compilation-request.h`; `zomlang/compiler/driver/package/canonical-package-compilation-request.cc` | handle-free canonical package request records, exact codecs, verified-request projection, and independent projection verifier; completed production ownership remains closed while RFC 0030 `R30-13` owns the comprehensive schema mutation test |
| `I1` | `module-system` with `runtime-memory` review | RFC 0029 `R29-14` | `zomlang/compiler/identity/canonical/identity-interner-set.h`; `zomlang/compiler/identity/canonical/identity-interner-set.cc`; `zomlang/compiler/query/semantic-context-capability-arena.h`; `zomlang/compiler/query/semantic-context-capability-arena.cc` | arena-owned eight-domain typed interner with collision, concurrency, reverse-lookup, and surviving-lease tests |
| `I1A` | `module-system` with `verification` review | `Q3`; RFC 0029 `R29-14` | `zomlang/compiler/driver/query/module-graph/module-graph-query-input.h`; `zomlang/compiler/driver/query/module-graph/module-graph-query-input.cc`; `zomlang/compiler/binder/stable-binding-schema.def`; `scripts/check-stable-binding-schema.py`; `zomlang/tests/unittests/compiler/driver/module-graph-query-input-test.cc` | prepare canonical input entries, the complete compilation-context authority value and exact codec, a producer-independent live-authority verifier, and the full declared mutation matrix; do not register or land the descriptor independently |
| `I2A` | `module-system` | `I1`; `I1A` review; RFC 0029 `R29-14`; `S5` | `zomlang/compiler/driver/query/binding/active-identity-membership-query.h`; `zomlang/compiler/driver/query/binding/active-identity-membership-query.cc` | prepare `ActiveMembershipResult`, its closed codec, record-codec contracts, and `ActiveCompilationUnitMembership`; do not land independently |
| `I2B` | `module-system` | `I2A` review | `zomlang/compiler/driver/query/binding/active-identity-membership-query.h`; `zomlang/compiler/driver/query/binding/active-identity-membership-query.cc` | prepare compilation-unit and crate membership descriptors with independent providers and verifiers; do not land independently |
| `I2C` | `module-system` | `I2B` review | `zomlang/compiler/driver/query/binding/active-identity-membership-query.h`; `zomlang/compiler/driver/query/binding/active-identity-membership-query.cc` | prepare source and module membership descriptors with active-parent validation and independent providers and verifiers; do not land independently |
| `I2D` | `module-system` | `I2C` review | `zomlang/compiler/driver/query/binding/active-identity-membership-query.h`; `zomlang/compiler/driver/query/binding/active-identity-membership-query.cc`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.h`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.cc` | prepare `CompleteRootIdentityReadiness`, the definition-authority input verifier, and definition membership with inventory-backed positive authority and conditional readiness; do not land independently |
| `I2E1` | `module-system` | `I2D` review | `zomlang/compiler/driver/query/binding/active-identity-membership-query.h`; `zomlang/compiler/driver/query/binding/active-identity-membership-query.cc`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.h`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.cc` | prepare the complete implementation membership record, exact codec, `ActiveImplementationAuthorityInput`, and structural input verifier; do not land independently |
| `I2E2` | `module-system` | `I2E1` review | `zomlang/compiler/driver/query/binding/active-identity-membership-query.h`; `zomlang/compiler/driver/query/binding/active-identity-membership-query.cc`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.h`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.cc` | prepare implementation occurrence coverage and independent membership provider and verifier starting from the exact implementation-authority input; do not land independently |
...
| `I2F1` | `module-system` | `I2E` review | `zomlang/compiler/driver/query/binding/active-identity-membership-query.h`; `zomlang/compiler/driver/query/binding/active-identity-membership-query.cc`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.h`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.cc` | prepare implementation-generic authority, the definition/implementation owner sum, complete generic-parameter membership records, exact codecs, `ActiveGenericParameterAuthorityInput`, and its structural verifier; do not land independently |
| `I2F2` | `module-system` | `I2F1` review | `zomlang/compiler/driver/query/binding/active-identity-membership-query.h`; `zomlang/compiler/driver/query/binding/active-identity-membership-query.cc`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.h`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.cc` | prepare definition-owned and implementation-owned generic-parameter membership providers and independent verifiers starting from the exact parameter-authority input and preserving equal-occurrence authority; do not land independently |
...
| `I2G1` | `module-system` | `I2F` review | `zomlang/compiler/driver/query/binding/active-identity-membership-query.h`; `zomlang/compiler/driver/query/binding/active-identity-membership-query.cc`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.h`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.cc` | prepare the complete callable-parameter membership record, exact codec, `ActiveCallableParameterAuthorityInput`, and its structural verifier; do not land independently |
| `I2G2` | `module-system` | `I2G1` review | `zomlang/compiler/driver/query/binding/active-identity-membership-query.h`; `zomlang/compiler/driver/query/binding/active-identity-membership-query.cc`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.h`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.cc` | prepare callable-parameter membership provider and independent verifier starting from the exact parameter-authority input with receiver and position validation; do not land independently |
| `I2G3` | `module-system` | `I2G2` review | `zomlang/compiler/driver/query/binding/active-identity-membership-query.h`; `zomlang/compiler/driver/query/binding/active-identity-membership-query.cc`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.h`; `zomlang/compiler/driver/query/binding/active-definition-authority-query.cc` | prepare registration for all four contextual authority inputs, complete-root readiness input, and eight membership descriptors; do not land independently |
...
| `B1` | `binder-checker` | RFC 0029 `R29-12AB`; RFC 0029 `R29-12D`; `S5` | `zomlang/compiler/binder/graph/module-skeleton-query.h`; `zomlang/compiler/binder/graph/module-skeleton-query.cc` | `BindModuleSkeleton`, projections, and independent verifier |
| `B2` | `binder-checker` | `B1` | `zomlang/compiler/binder/surface/owner-body-query.h`; `zomlang/compiler/binder/surface/owner-body-query.cc` | contextual `BindOwnerBody` and independent traversal/verifier |
| `B3` | `binder-checker` | `B2` | `zomlang/compiler/binder/surface/owner-body-syntax.h`; `zomlang/compiler/binder/surface/owner-body-syntax.cc` | complete node-scope, capture, control, and provenance detachment |
| `B4` | `binder-checker` | `B2`; `B3` | `zomlang/compiler/binder/graph/module-binding-allocation-plan.h`; `zomlang/compiler/binder/graph/module-binding-allocation-plan.cc` | deterministic five-domain allocation plan |
| `M1A1` | `module-system` | `I2` review; RFC 0029 `R29-14`; RFC 0028 `R28-16` | `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.h` | prepare the public Pimpl declarations for every non-trivial materialized graph value, the explicit identity-entry instantiation aliases, and the descriptor declaration; do not land independently |
| `M1A2` | `module-system` | `M1A1` review | `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.cc` | prepare private Pimpl storage, move operations, destructors, clones, and basic accessors for the M1A1 values; do not land independently |
| `M1A3` | `module-system` | `M1A2` review | `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.h`; `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.cc` | prepare the four-domain identity-resource interface, exact active-materialization specializations, closed interner-failure mapping, and four compile-time permissions; do not land independently |
...
| `M1B` | `module-system` | `M1A` review | `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.h`; `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.cc` | prepare stable dependency and graph witness construction, exact codecs, comparison, and candidate contract; do not land independently |
| `M1C1A` | `module-system` | `M1B` review | `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.cc` | prepare provider-only complete-context and fingerprint-prerequisite reads, active crate/source/module acquisition, and graph/SCC validation in canonical order; do not issue per-module dependency or capability reads and do not land independently |
| `M1C1B` | `module-system` | `M1C1A` review | `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.cc` | prepare provider-only per-module selected-source, dependency-site, request, exact-resolution, configured-prelude, parse, and provenance reads plus exact typed child-failure forwarding in canonical order; do not materialize or land independently |
...
| `M1C2` | `module-system` | `M1C1` review | `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.cc` | prepare provider-only four-domain membership admission, logical-const materialization, and reverse expansion in the accepted domain order; do not publish a candidate or land independently |
| `M1C3` | `module-system` | `M1C2` review | `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.cc` | prepare provider-only stable witness, independently recomputed revision, handle-edge, and candidate publication from the approved ordered acquisition and materialization state; do not land independently |
...
| `M1D1` | `module-system` | `M1C` review | `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.cc` | prepare verifier-only exact context, fingerprint-prerequisite, active-domain, graph, and SCC acquisition with independent canonical ordering; do not issue per-module reads, materialize, or land independently |
| `M1D2` | `module-system` | `M1D1` review | `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.cc` | prepare verifier-only per-module resolution, reached prelude, parse, provenance, independent stable-edge ordering, and fingerprint reconstruction; do not materialize or land independently |
| `M1D3` | `module-system` | `M1D2` review | `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.cc` | prepare verifier-only four-domain membership materialization, same-resource reverse expansion, independent revision and graph-edge reconstruction, handle-edge validation, and candidate witness comparison; do not issue earlier reads or land independently |
...
| `M1E` | `module-system` | `M1D` review | `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.h`; `zomlang/compiler/driver/query/module-graph/materialized-module-graph-query.cc` | prepare source/key failure contracts plus schema-derived capability, failure, permission, and descriptor assertions; do not land independently |
...
| `M2` | `binder-checker` | `B4`; `T1` | `zomlang/compiler/binder/graph/materialized-module-skeleton.h`; `zomlang/compiler/binder/graph/materialized-module-skeleton.cc` | typed skeleton expansion and provenance after the complete-context atomic landing |
| `M3` | `binder-checker` | `B4`; `M2` | `zomlang/compiler/binder/materialized-owner-body.h`; `zomlang/compiler/binder/materialized-owner-body.cc` | typed body expansion without diagnostic references |
| `M4` | `binder-checker` | `M2`; `M3` | `zomlang/compiler/binder/metadata/immutable-definition-inventory.h`; `zomlang/compiler/binder/metadata/immutable-definition-inventory.cc` | complete owned Checker identity and node lookup index |
| `M5` | `binder-checker` | `M4` | `zomlang/compiler/binder/verified-bound-module.h`; `zomlang/compiler/binder/verified-bound-module.cc` | `VerifyBoundModule`, aggregate coverage, failure projection, and retained children |
| `C1` | `binder-checker` | `M5` | `zomlang/compiler/checker/module-interface-contract.h`; `zomlang/compiler/checker/module-interface-contract.cc` | lease-owning Checker view and interface contract |
| `C1A` | `binder-checker` | `C1` | `zomlang/compiler/checker/body/body-checker.h`; `zomlang/compiler/checker/body/body-checker.cc` | migrate Checker body consumers |
| `C2` | `module-system` | `C1`; `C1A` | `zomlang/compiler/driver/interface/module-interface.h`; `zomlang/compiler/driver/interface/module-interface.cc` | module-interface publication from the lease-owning view |
| `L1` | `ir-backend` | `C1A` | `zomlang/compiler/hir/checked-module.h`; `zomlang/compiler/hir/checked-module.cc` | checked-module retained lease and verifier |
| `L2` | `ir-backend` | `L1` | `zomlang/compiler/hir/hir-module.h`; `zomlang/compiler/hir/hir-module.cc` | HIR retained lease and lineage verifier |
| `L3` | `ir-backend` | `L2` | `zomlang/compiler/mir/built-mir.h`; `zomlang/compiler/mir/built-mir.cc` | Built MIR retained lease and lineage verifier |
| `L4` | `runtime-memory` | `L3` | `zomlang/compiler/ownership/ownership-event-overlay.h`; `zomlang/compiler/ownership/ownership-event-overlay.cc` | ownership-overlay retained lease, verifier, and destruction order |
| `T1` | `module-system` with `verification` and query-runtime review | approved I1A, I2, and M1 preparations; RFC 0029 `R29-14`; RFC 0028 `R28-16` | all files listed by I1A, I2, and M1; `zomlang/compiler/binder/binding-input.h`; `zomlang/compiler/binder/binding-input.cc`; `zomlang/compiler/identity/semantic/context-fingerprint.h`; `zomlang/compiler/identity/semantic/context-fingerprint.cc`; `zomlang/compiler/query/query-database.h`; `zomlang/compiler/query/query-descriptor-schema.def`; `zomlang/compiler/driver/CMakeLists.txt`; `zomlang/compiler/driver/core-library-query-provider.h`; `zomlang/compiler/driver/core-library-query-provider.cc`; `zomlang/compiler/driver/query/binding/active-definition-authority-session.h`; `zomlang/compiler/driver/query/binding/active-definition-authority-session.cc`; `zomlang/compiler/driver/session/compiler-session.cc`; `zomlang/tests/unittests/compiler/binder/binding-input-test.cc`; `zomlang/tests/unittests/compiler/driver/core-library-query-provider-test.cc`; `zomlang/tests/unittests/compiler/driver/active-definition-authority-session-test.cc`; `zomlang/tests/unittests/compiler/driver/compiler-session-package-test.cc`; `zomlang/tests/unittests/compiler/query/query-test-specs.h`; `zomlang/tests/unittests/compiler/query/query-test-descriptor-schema.def`; `zomlang/tests/unittests/compiler/query/query-database-test.cc`; `zomlang/tests/unittests/compiler/query/query-capability-test.cc`; `zomlang/tests/unittests/compiler/query/CMakeLists.txt`; `scripts/check-query-descriptor-architecture.py`; `scripts/generate-query-descriptor-schema.py` | atomically land the complete-context value and descriptor, the unique graph-revision and fingerprint canonical-digest construction boundaries, the narrow capability-context revision and typed-resource accessors, memberships, readiness, graph and SCC witnesses, the production and test descriptor inventories, their exact driver build rows, three static transaction-witness descriptors, three closed input transactions with caller-supplied previous revisions and same-revision witnesses, all three production mutation caller cutovers including direct replacement of authority refresh, both same-name shadow deletions, the slot-59 generic runtime fixture, production descriptor final-seal integration, complete static final-authority verification, full mutation matrix, and staging, final, and sealed snapshot publication |
| `T2A` | `module-system` | `M5`; `T1` | `zomlang/compiler/driver/session/compiler-session.h`; `zomlang/compiler/driver/session/compiler-session.cc` | install the transaction state machine and named snapshots, and make the arena-owned compiler-session semantic resource implement the approved module-graph identity-materialization interface without a second interner or resource fallback |
| `T2B` | `module-system` | `C2`; `L4`; `T2A` | `zomlang/compiler/driver/session/compiler-session.h`; `zomlang/compiler/driver/session/compiler-session.cc` | dependency-first final capability root and irreversible seal |
| `T2C` | `module-system` | `T2B` | `zomlang/compiler/driver/session/compiler-session.h`; `zomlang/compiler/driver/session/compiler-session.cc` | surviving-lease and session teardown order |
| `D1` | `module-system` | `T2C` | `zomlang/compiler/identity/semantic-identity-registry-set.h`; `zomlang/compiler/identity/semantic-identity-registry-set.cc`; `zomlang/compiler/identity/frozen-registry.h` | delete registry/freeze identity authority |
| `D2` | `binder-checker` | `T2C` | `zomlang/compiler/binder/frozen-definition-inventory.h`; `zomlang/compiler/binder/frozen-definition-inventory.cc` | delete frozen inventory and its lookup view |
| `D3` | `module-system` | `T2C` | `zomlang/compiler/driver/query/binding/incremental-binding-query-adapter.h`; `zomlang/compiler/driver/query/binding/incremental-binding-query-adapter.cc`; `zomlang/compiler/driver/query/module-graph/module-graph-query.h`; `zomlang/compiler/driver/query/module-graph/module-graph-query.cc` | delete session ledgers and handleful graph root |
| `D4` | `binder-checker` | `T2C` | `zomlang/compiler/binder/binding-run.h`; `zomlang/compiler/binder/binding-run.cc`; `zomlang/compiler/binder/verified-bound-module-input.h`; `zomlang/compiler/binder/verified-bound-module-input.cc` | delete production batch root and non-owning input |
| `D5` | `ir-backend` | `L3` | `zomlang/compiler/ir/ir-failure.h`; `zomlang/compiler/ir/ir-failure.cc`; `zomlang/compiler/ir/ir-diagnostic-adapter.h`; `zomlang/compiler/ir/ir-diagnostic-adapter.cc` | delete Binder-to-IR failure conversion and non-owning IR lineage |
| `W1` | `binder-checker` | `C1`; `D4`; `M5` | `zomlang/compiler/binder/CMakeLists.txt`; `zomlang/compiler/checker/CMakeLists.txt` | Binder and Checker source wiring |
| `W2` | `module-system` | `D1`; `D3`; `T2C` | `zomlang/compiler/identity/CMakeLists.txt`; `zomlang/compiler/query/CMakeLists.txt`; `zomlang/compiler/driver/CMakeLists.txt` | post-T2C identity, query, graph, and session deletion/final wiring without changing the two T1-owned additive source rows |
| `W3` | `ir-backend` | `D5`; `L3` | `CMakeLists.txt`; `CMakePresets.json`; `zomlang/compiler/CMakeLists.txt`; `zomlang/compiler/hir/CMakeLists.txt`; `zomlang/compiler/mir/CMakeLists.txt`; `zomlang/compiler/ir/CMakeLists.txt` | compiler and IR build wiring |
| `W4` | `runtime-memory` | `L4` | `zomlang/compiler/ownership/CMakeLists.txt` | ownership-overlay source wiring |
| `E1` | `verification` | `W1`; `W2`; `W3`; `W4` | `zomlang/tests/unittests/compiler/binder/stable-binding-query-test.cc`; `zomlang/tests/unittests/compiler/binder/materialized-binding-capability-test.cc`; `zomlang/tests/unittests/compiler/query/query-capability-test.cc`; `zomlang/tests/unittests/compiler/driver/compiler-session-test.cc`; `zomlang/tests/CMakeLists.txt` | stable, materialized, seal, teardown, and cross-owner lineage tests |
| `E2` | `verification` | `E1` | `zomlang/tests/unittests/compiler/hir/hir-module-test.cc`; `zomlang/tests/unittests/compiler/mir/built-mir-test.cc`; `zomlang/tests/unittests/compiler/ownership/ownership-event-overlay-test.cc`; `zomlang/tests/unittests/compiler/ir/ir-failure-test.cc`; `zomlang/tests/unittests/compiler/ir/ir-diagnostic-adapter-test.cc`; `zomlang/tests/unittests/compiler/driver/incremental-binding-query-adapter-test.cc` | Checked/HIR/MIR/ownership joint lineage and deletion regressions |
...
| `E5` | `verification` | `E4` | `scripts/generate-canonical-header-syntax-schema.py`; `scripts/codegen/gen_ast.py`; `scripts/codegen/gen_core_library_inventory.py`; `scripts/codegen/gen_package_oracles.py`; `zomlang/compiler/binder/stable-binding-schema.def` | generated header, AST, core-library, and package-oracle drift plus hand-authored stable-binding schema consumer drift and self-tests |
| `E6` | `verification` | `G4`; `E3` | `scripts/run-ownership-coverage.py`; `scripts/check-ownership-coverage.py`; `zomlang/tests/coverage/ownership-exemptions.json`; `zomlang/tests/coverage/implementation-series-base.txt`; `cmake/utils/coverage.cmake` | frozen-base ownership coverage artifacts and thresholds |
| `E7` | `verification` | `E3`; `E4`; `E4A`; `E5`; `E6`; `W1-W4` | `.github/workflows/CI.yml`; `zomlang/tests/CMakeLists.txt`; `zomlang/tests/integration/core-library/installed-consumer/Zom.toml`; `zomlang/tests/integration/core-library/installed-consumer/src/main.zom`; `zomlang/tests/cmake/verify-core-library-install-consumer.cmake`; `cmake/utils/unittests.cmake` | native sanitizer, codegen, gate, coverage, and the unique `core-library-install-consumer` integration |
| `E8` | `verification` | `E7` | `scripts/run-incremental-query-benchmarks.py`; `zomlang/tests/performance/incremental-query-corpus.json`; `zomlang/tests/performance/incremental-query-baseline.json` | fixed Release compare and worker-determinism evidence |
...
- Ownership coverage:
  ...
  artifacts and
  `zomlang/tests/coverage/ownership-exemptions.json` and
  `zomlang/tests/coverage/implementation-series-base.txt`, requires at
