...
| 3 | Partial | Binder now defers unresolved member expressions to the checker, and `BodyChecker` resolves concrete class method members through the class scope in `SymbolTable`. `body-checker-test.cc` verifies `Counter.value()` records an `InstanceMethod` target with `ImplicitSelf`, target symbol ID, receiver type ID, and result type ID, and verifies `Counter.make()` records a `StaticMethod` target with `ReceiverMode::None`, target symbol ID, no receiver argument, and result type ID. A direct `zomc compile --syntax-only /tmp/member_call.zom` regression check also succeeds. | Extend coverage to struct methods, imported methods, and ambiguous/missing concrete method diagnostics. |
