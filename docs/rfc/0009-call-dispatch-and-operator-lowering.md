...
| 3 | Not Started | Audit evidence: parser produces `MemberExpression` for `c.value()`, but the current binder still reports `Undefined identifier: 'value'` before checker dispatch can run. | Fix member lookup/binder support for concrete method calls, then record `InstanceMethod` and `StaticMethod` targets. |
