...
| 11 | Partial | `TypeEnv::dumpDispatch()` emits a deterministic `zom.dispatch.v0` debug text stream sorted by expression `NodeId`. `type-env-test.cc` covers empty output, ordering independent from hash-map iteration, target/receiver names, target symbol IDs, interface/method names, vtable slots, argument type IDs, and result type IDs. | Add the user-facing debug flag after the CLI output contract is accepted, and add IR-side debug snapshots once a lowering consumer exists. |
