...
| 3 | Partial | Binder now defers unresolved member expressions to the checker, and `BodyChecker` resolves concrete class and struct method members through the class/struct scope in `SymbolTable`. `body-checker-test.cc` verifies `Counter.value()` and `Point.norm()` record `InstanceMethod` targets with `ImplicitSelf`, target symbol ID, receiver type ID, and result type ID, and verifies `Counter.make()` records a `StaticMethod` target with `ReceiverMode::None`, target symbol ID, no receiver argument, and result type ID. CLI conformance covers class instance, class static, and struct instance member calls through `member_call_instance_pos_16`, `member_call_static_pos_17`, and `member_call_struct_pos_18`; `member_call_missing_neg_19` covers the user-visible `ZOM4037` missing-member diagnostic without a cascading `ZOM4012`. | Extend coverage to imported methods after RFC 0008 module signature publication exists. Ambiguous concrete method diagnostics are not a checker dispatch surface in v1 because duplicate same-scope method declarations are rejected by the binder before dispatch selection. |
