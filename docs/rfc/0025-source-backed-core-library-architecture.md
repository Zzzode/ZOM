...
`core`, but it is packaged as a normal `zomcore` release with the
...
flowchart LR
    ...
    S["core/src/*.zom"] --> F["Normal frontend pipeline"]
    C --> F
...
`core/src` is the canonical repository source root and
...
The stable Binder schema, facts, and codecs remain separate bounded review
partitions but land atomically through the exact RFC 0030 `R29-12AB`
transaction. The diagnostic transaction `R29-12D` lands separately after that
...
The implementation removes `core/Zom.toml`. No replacement
...
3. Replace `core/Zom.toml` with the fixed source-root admission and
...
- Rewrite `core/README.md` as the contributor guide for source
...
- Architecture: register
  ...
  `core/Zom.toml`, core SemVer, empty production preludes,
