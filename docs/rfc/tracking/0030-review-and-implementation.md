...
| `R41-15F1A` | `binder-checker` with `verification` review | `R41-14D` | In the fact files, implement the complete `BoundOwnerBody` value and move-only compile evidence; at most 400 changed source lines. | Pimpl, factory, indexes, accessors, and exact hashes | Pending |
| `R41-15F1B` | `binder-checker` with `verification` review | `R41-15F1A` | In the native test only, add populated production-built aggregate smoke evidence; at most 400 changed source lines. | Every component family, every accessor, exact predecessor, and line cap | Pending |
| `R41-15F2A` | `binder-checker` with `verification` review | `R41-15F1B` | In the native test only, add structural and relational aggregate evidence; at most 400 changed source lines. | Exact test predecessor and line cap | Pending |
