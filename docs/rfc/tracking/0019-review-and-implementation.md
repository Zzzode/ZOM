...
|---|---|---|---|
| 0 | RFC 0029 synchronized prerequisites and runtime replacement | Complete | RFC 0029 `R29-12A` through `R29-17` landed the synchronized schema, diagnostics, runtime replacement, native evidence, Release comparison, and current design |
...
| 2 | Module body syntax and provenance | Complete | Selected-source, detached syntax, provenance, dependency, source-switch, session, and architecture tests passed. The RFC 0029 query-runtime and transaction follow-up is complete; synchronized header work remains under RFC 0027 `S4` through `S5`. |
| 3 | Owner-body query catalog and aggregate verification | In progress | Syntax, provenance, and owner census are implemented, and the RFC 0029 runtime prerequisites are complete. Stable Binder queries, allocation, materializers, aggregate verification, and production demand remain under RFC 0027 `B1` through `B4` and `M2` through `M5`. |
