...
| `R27-14A` | `binder-checker` with `module-system` integration and `verification` review | RFC 0029 `R29-12AB`; RFC 0029 `R29-14` | Land typed complete definition and implementation inventory entries, exact codecs and mutation coverage, and direct production caller cutover. | Focused inventory and caller tests | Complete; `query-binding-named-identity-inventory-query-test` and `query-binding-incremental-binding-query-adapter-test` passed under the sanitizer preset on 2026-08-02 |
| `R27-14B` | `module-system` with `binder-checker` and `verification` review | `R27-14A` | Make the named-definition inventory query read selected parse provenance, derive body disposition independently in provider and verifier, and prove exact read ordering and failure behavior. | Focused query tests and `python3 scripts/check-binder-architecture.py --self-test` | Complete; focused inventory caller tests and the binder architecture self-test passed on 2026-08-02 |
| `R27-15C` | `binder-checker` with `verification` review | `R27-14B` | Implement `DefinitionHeaderProducer` from the closed borrowed input record, retained parse provenance, and complete syntax-role matrix. | Definition-header producer tests and cross-input mismatch mutations | Complete; `stable/definition/header-producer-test` passed under the sanitizer preset on 2026-08-02 |
| `R27-15D` | `binder-checker` with `verification` review | `R27-14B` | Implement `ImplementationHeaderProducer` from the closed borrowed input record with complete identity, source-form, and scope-role coverage. | Implementation-header producer tests and cross-input mismatch mutations | Complete; `stable/implementation/header-producer-test` passed under the sanitizer preset on 2026-08-02 |
| `R27-15E` | `binder-checker` with `verification` review | `R27-15C`; `R27-15D` | Implement `StableHeaderVerifier`, independently select entries and sites from complete inventories and projections, synchronize schema provenance names, and cover equal occurrences without producer traversal reuse. | Header verifier, schema, caller-selected-entry, and disagreement mutations | Complete; `stable/header/verifier-test` and the stable-binding schema checks passed under the sanitizer preset on 2026-08-02 |
...
| `R27-18` | `module-system` with required `runtime-memory` review | RFC 0029 `R29-14` | Replace frozen registries with the arena-owned eight-domain typed interner set. | Identity, concurrency, collision, and teardown tests | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-18A` | `module-system` with `verification` review | `R27-17A`; RFC 0029 `R29-14` | Prepare canonical input entries, `CompleteCompilationContextAuthority`, its exact codec, descriptor definition, producer-independent verifier, schema ownership and producer identity, and the full declared mutation matrix; do not register or land independently. | Complete-context codec, verifier, mutation, and schema review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19A` | `module-system` | `R27-15E`; RFC 0029 `R29-14`; `R27-18`; approved `R27-18A` preparation | Prepare `ActiveMembershipResult`, its closed codec and record-codec contracts, and `ActiveCompilationUnitMembership`; do not land independently. | Result, codec, and compilation-unit record review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19B` | `module-system` | approved `R27-19A` preparation | Prepare compilation-unit and crate membership descriptors with independent providers and verifiers; do not land independently. | Compilation-unit and crate membership review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19C` | `module-system` | approved `R27-19B` preparation | Prepare source and module membership descriptors with active-parent validation and independent providers and verifiers; do not land independently. | Source and module membership review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19D` | `module-system` | approved `R27-19C` preparation | Prepare complete-root readiness, the definition-authority input verifier, and inventory-backed definition membership; do not land independently. | Readiness, definition authority, and conditional absence review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19E1` | `module-system` | approved `R27-19D` preparation | Prepare the complete implementation membership record, exact codec, `ActiveImplementationAuthorityInput`, and structural input verifier; do not land independently. | Implementation record, codec, and input review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19E2` | `module-system` | approved `R27-19E1` preparation | Prepare implementation occurrence coverage and independent membership provider and verifier starting from the exact implementation-authority input; do not land independently. | Implementation occurrence and membership review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19E` | `module-system` | approved `R27-19E1` and `R27-19E2` preparations | Review the complete implementation membership union without source edits or independent landing. | Complete implementation membership review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19F1` | `module-system` | approved `R27-19E` preparation | Prepare implementation-generic authority, the definition/implementation owner sum, complete generic-parameter membership records, exact codecs, `ActiveGenericParameterAuthorityInput`, and its structural verifier; do not land independently. | Generic records, owner sum, codec, and input review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19F2` | `module-system` | approved `R27-19F1` preparation | Prepare definition-owned and implementation-owned generic-parameter membership providers and independent verifiers starting from the exact parameter-authority input and preserving equal-occurrence authority; do not land independently. | Generic owner and occurrence review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19F` | `module-system` | approved `R27-19F1` and `R27-19F2` preparations | Review the complete generic-parameter membership union without source edits or independent landing. | Complete generic membership review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19G1` | `module-system` | approved `R27-19F` preparation | Prepare the complete callable-parameter membership record, exact codec, `ActiveCallableParameterAuthorityInput`, and its structural verifier; do not land independently. | Callable record, codec, and input review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19G2` | `module-system` | approved `R27-19G1` preparation | Prepare callable-parameter membership provider and independent verifier starting from the exact parameter-authority input with receiver and position validation; do not land independently. | Callable header, receiver, and position review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19G3` | `module-system` | approved `R27-19G2` preparation | Prepare registration for all four contextual authority inputs, complete-root readiness input, and eight membership descriptors; do not land independently. | Complete authority and membership registration review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19G` | `module-system` | approved `R27-19G1` through `R27-19G3` preparations | Review the complete callable-parameter and registration union without source edits or independent landing. | Complete callable and registration review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-19` | `module-system` with `verification` review | approved preparations `R27-19A` through `R27-19G` | Review the complete eight exact complete-record memberships and conditional readiness union without source edits or independent landing. | Complete membership, readiness, and mutation-inventory review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-20` | `binder-checker` | RFC 0029 `R29-12AB`; RFC 0029 `R29-12D`; `R27-15E` | Implement `BindModuleSkeleton`, lookup projections, and independent verifier. | Complete: sealed-snapshot skeleton aggregation, stable codec and descriptor registration, module-path scope coverage, end-to-end publication checks, `ModuleExportNames`, `ImplementationBindingHeader`, `DefinitionBindingHeader`, `ExportedBinding`, `ScopeNameBucket`, `BindingVisibility`, and `ImportTarget` are implemented. Direct declaration exports, exact foreign-reexport provenance, and module-alias semantic slots with target-surface revisions populate the skeleton. Final authority admission for a module-alias source and independently derived import, alias, and export verification are covered end to end. | Complete |
| `R27-21` | `binder-checker` | `R27-20` | Implement contextual `BindOwnerBody` and independent traversal/verifier. | Body, capture, control, and source-failure tests | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-22` | `binder-checker` | `R27-21` | Implement the deterministic five-domain module allocation plan. | Overflow and reversed-demand tests | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23A1` | `module-system` | RFC 0029 `R29-14`; RFC 0028 `R28-16`; approved `R27-19` preparation | Prepare public Pimpl declarations for every non-trivial materialized graph value, explicit identity-entry aliases, and the descriptor declaration; do not land independently. | Value-shape, opaque-layout, and header-boundary review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23A2` | `module-system` | approved `R27-23A1` preparation | Prepare private Pimpl storage, explicit identity-entry instantiations, move operations, destructors, clones, and basic accessors; do not land independently. | Pimpl, ownership, move-only, and accessor review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23A3` | `module-system` | approved `R27-23A2` preparation | Prepare the four-domain identity-resource interface, exact active-materialization specializations, closed interner-failure mapping, and four permissions; do not land independently. | Resource, permission, and interner-failure review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23A` | `module-system` | approved preparations `R27-23A1`; `R27-23A2`; `R27-23A3` | Review the complete M1A union without adding files or landing it. | Opaque value, resource, permission, and no-fallback review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23B` | `module-system` | approved `R27-23A` preparation | Prepare stable dependency and graph witness construction, exact codecs, canonical comparison, and candidate contract; do not land independently. | Codec, exact-consumption, closure, and revision review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23C1A` | `module-system` | approved `R27-23B` preparation | Prepare provider-only complete-context and fingerprint-prerequisite reads, active crate/source/module acquisition, and graph/SCC validation in canonical order; do not issue per-module dependency or capability reads and do not land independently. | Complete-context, fingerprint-input, active-domain, graph, and SCC review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23C1B` | `module-system` | approved `R27-23C1A` preparation | Prepare provider-only per-module selected-source, dependency-site, request, exact-resolution, configured-prelude, parse, and provenance reads plus exact typed child-failure forwarding in canonical order; do not materialize or land independently. | Module read-set, rejection-order, parse, and provenance review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23C1` | `module-system` | approved `R27-23C1A` and `R27-23C1B` preparations | Review the complete provider acquisition union without adding files or landing it. | Complete acquisition-order and typed-failure review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23C2` | `module-system` | approved `R27-23C1` preparation | Prepare provider-only four-domain membership admission, logical-const materialization, and reverse expansion in the accepted domain order; do not publish a candidate or land independently. | Membership, resource, domain-order, and reverse-expansion review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23C3` | `module-system` | approved `R27-23C2` preparation | Prepare provider-only stable witness, independently recomputed revision, handle-edge, and candidate publication from the approved ordered acquisition and materialization state; do not land independently. | Witness, revision, handle-edge, and candidate-publication review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23C` | `module-system` | approved `R27-23C1` through `R27-23C3` preparations | Review the complete provider union without adding files or landing it. | Complete read-set, rejection-order, membership, reverse-expansion, and publication review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23D1` | `module-system` | approved `R27-23C` preparation | Prepare verifier-only exact context, fingerprint-prerequisite, active-domain, graph, and SCC acquisition with independent canonical ordering; do not issue per-module reads, materialize, or land independently. | Independent context, graph, SCC, and order review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23D2` | `module-system` | approved `R27-23D1` preparation | Prepare verifier-only per-module resolution, reached prelude, parse, provenance, independent stable-edge ordering, and fingerprint reconstruction; do not materialize or land independently. | Independent module read-set, edge order, provenance, and fingerprint review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23D3` | `module-system` | approved `R27-23D2` preparation | Prepare verifier-only four-domain membership materialization, same-resource reverse expansion, independent revision and graph-edge reconstruction, handle-edge validation, and candidate witness comparison; do not issue earlier reads or land independently. | Independent membership, reverse, revision, edge, and candidate review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23D` | `module-system` | approved `R27-23D1` through `R27-23D3` preparations | Review the complete verifier union without adding files or landing it. | Complete independent reconstruction, demand-order, and mutation review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23E` | `module-system` | approved `R27-23D` preparation | Prepare source/key failure contracts plus schema-derived capability, failure, permission, and descriptor assertions; do not land independently. | Failure-envelope, schema, permission, and descriptor review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-23` | `module-system` | approved preparations `R27-23A`; `R27-23B`; `R27-23C`; `R27-23D`; `R27-23E` | Review the complete M1 union without adding files or landing it. | Graph, provenance, membership projection, canonical authority comparison, reverse expansion, resource-interface, total rejection-order, provider/verifier independence, and read-set review | Complete; atomically landed by `6fac75b4` and `eb1033ef`; the current 10-target sanitizer audit passed on 2026-08-07. |
| `R27-24` | `binder-checker` | `R27-22`; `R27-28A` | Complete: `MaterializeModuleSkeleton` and `MaterializeOwnerBody` publish final-sealed retained capabilities, forward typed upstream rejection, independently verify their current stable witnesses, and require the skeleton's complete eight-domain active-membership gate before owner-body publication. The current implementation retains the materialized module-graph, revision-local definition-site, and implementation-site leases, one exact named-item provenance lease per `ModuleSkeleton` declaration, materializes skeleton DefinitionFact, GenericParameterFact, CallableParameterFact, ModuleAliasBindingFact, direct and local-reexport LocalExportFact, ImplBindingFact, NodeScopeFact, ScopeRecord, scope identities, and owner-body node-to-scope, owner-local binding, runtime name-resolution, contextual self type, receiver, shadow-target, failed-lookup, deferred-member, label, control-transfer, and explicit closure-capture facts with exact source, DefinitionSite, scope, kind, name, namespace, activation, pattern ancestry, and retained syntax paths. The skeleton now retains an independently verified local binding surface derived only from sealed witnesses, materialized definition identities, and local export facts; it includes direct exports and local re-exports with exact alias spans and re-export provenance. Scope records reconstruct module, definition, and implementation owners, parent edges, declaration, generic, named callable, module-alias canonical-module, import, and foreign re-export bindings from retained materialized identities and source spans; failed lookups remain exact independent syntax projections. Header membership-withdrawal coverage now rejects removal of definition, implementation, generic-parameter, or callable-parameter authority before M2 publication. `BindOwnerBody` now retains direct value-identifier and single-segment named-type-path resolution and failure witnesses, produces nonempty deferred-member facts, resolves those sites through lexical owner scopes, projects owner-local shadow targets through nested scopes, projects explicit function-expression captures by value and reference through their enclosing lexical scope, projects `this` captures to the owning callable receiver, and projects `this` expressions to exactly one callable receiver, with local declarations activated after their initializer, skeleton declarations, generic parameters, callable parameters, imports, and a canonical missing, namespace-mismatch, or ambiguous failure projection. `MaterializeOwnerBody` converts addressable current-skeleton definitions, generic and callable parameters, current-module bindings, semantic-import keys, owner-local bindings, receiver witnesses, shadow targets, failed lookups, labels, explicit closure captures, and explicit or loop control transfers into runtime facts. Labels use retained provenance, owner allocation label ranges, and retained scope identities; cross-owner and cross-module handle targets remain fail-closed. The lookup verifier independently rebuilds scope ancestry and candidates in reverse traversal order, the shadow verifier independently rebuilds ancestor-scope targets, the explicit-capture verifier independently reprojects function-expression capture rows and lexical targets, receiver materialization is checked against the retained callable-parameter identity, and failed-lookup materialization reprojects each stable body-owned syntax path and outcome without a diagnostic reference; coverage includes missing and namespace-mismatch outcomes plus labeled-loop control-transfer, nested-local-shadow materialization, direct missing-shadow mutation rejection, and by-value, by-reference, `this`, and empty explicit captures. The typed M2/M3 expansion now materializes and independently verifies skeleton imports, foreign re-exports, failed lookups, dependency binding surfaces, owner-body multi-segment type paths, and the retained current fact domains. | Typed expansion and retained-child tests | Complete; focused sanitizer tests passed on 2026-08-04 |
| `R27-25` | `binder-checker` | `R27-24` | Implement `VerifyBoundModule`, immutable aggregate storage, and failure projection. | Coverage, lineage, and failure tests | Complete; retained aggregate lineage revalidates every skeleton and owner-body materialized fact domain. `ImmutableDefinitionInventory` owns complete NodeId projections for definitions, parameters, owner locals, role-qualified anonymous entities, and implementation occurrences. `ImmutableBindingMetadata` now owns the complete current node-scope, resolution, contextual, definition, scope, import/export, deferred-member, control, closure, parameter, owner-local, and failed-lookup fact aggregate; provider and verifier rebuild it from retained materialized children. Focused sanitizer capability and mutation tests passed on 2026-08-04. |
| `R27-26` | `binder-checker` | `R27-25` | Complete: Checker fact builders consume `CheckerBoundModuleView`, while `CheckerIdentityAuthority` retains one dependency-ordered verified view per materialized graph module and resolves the complete eight-domain identity authority. | Focused Checker tests | Complete; focused authority coverage plus the 39-test sanitizer `session-compiler-session-package-test` suite passed on 2026-08-05. |
| `R27-26A` | `module-system` | `R27-26` | Complete: module-interface publication owns and retains an `AdmittedBoundModule`. | Focused interface tests | Complete; module-interface teardown retention coverage passed on 2026-08-05. |
| `R27-27A` | `ir-backend` | `R27-26` | Complete: verified checked modules retain an `AdmittedBoundModule`. | Checked-module lineage tests | Complete; package pipeline lineage coverage passed on 2026-08-05. |
| `R27-27B` | `ir-backend` | `R27-27A` | Complete: verified HIR retains an `AdmittedBoundModule`. | HIR lineage tests | Complete; scalar-return HIR lineage coverage passed on 2026-08-05. |
| `R27-27C` | `ir-backend` | `R27-27B` | Complete: Built MIR retains an `AdmittedBoundModule`. | MIR lineage tests | Complete; scalar-return MIR lineage coverage passed on 2026-08-05. |
| `R27-27D` | `runtime-memory` | `R27-27C` | Complete: ownership overlays retain an `AdmittedBoundModule` and session teardown releases them first. | Ownership lineage tests | Complete; teardown order and package pipeline coverage passed on 2026-08-05. |
| `R27-28A` | `module-system` with `verification` and query-runtime review | RFC 0029 `R29-14`; RFC 0028 `R28-16`; approved preparations `R27-18A`, `R27-19`, and `R27-23` | Atomically land the complete-context value and descriptor, unique graph-revision and fingerprint construction boundaries, narrow capability-context revision and typed-resource accessors, memberships, readiness, graph and SCC witnesses, production and test descriptor inventories, their two exact driver CMake source rows, three static transaction-witness descriptors, complete static final verifier, three caller-revision-bound transactions with same-revision witnesses, all three production mutation caller cutovers, query-test migration, both shadow deletions, full mutation matrix, and staging, final, and sealed snapshots; the first transaction is replaced directly in `core-library-query-provider.{h,cc}` with native coverage in `core-library-query-provider-test.cc`, `compiler-session.cc` and `compiler-session-package-test.cc` own the buildable caller migration including direct deletion of the authority refresh path, and `scripts/generate-query-descriptor-schema.py` enforces all witness rows plus the ordinal-59 test-tail boundary. | Complete: exact complete-context and transaction-witness descriptors, final-authority verification, final-seal admission, context-scoped memberships, and caller cutovers are present. Descriptor schema and architecture self-tests, CompilerSession architecture self-test, focused authority/session/core/query tests, and the sanitizer suite passed after installing the required lit `psutil` dependency. | Complete |
| `R27-28B` | `module-system` | `R27-26A`; `R27-27D`; `R27-28A` | Complete: the arena-owned compiler-session semantic resource delegates four-domain interning and reverse lookup to its sole arena-owned interner. The dependency-first production root now demands the sealed materialized graph and every `VerifyBoundModule` capability in canonical graph order, including direct module imports without requiring a definition export entry. | Resource ownership, no-fallback, session architecture, and end-to-end tests | Complete; focused materialization, checker rejection, and full `session-compiler-session-package-test` sanitizer coverage passed on 2026-08-05, together with both CompilerSession architecture gates. |
| `R27-28C` | `module-system` | `R27-28B` | Complete: `CompilerSession::Impl` explicitly releases ownership overlays, Built MIR, HIR, checker facts, checker authority, retained bound-module mirrors, and sealed snapshots in reverse dependency order before member destruction releases the query database and session arena. | Teardown tests | Complete; checker-authority and module-interface leases remain valid after complete session teardown, and full `session-compiler-session-package-test` sanitizer coverage passed on 2026-08-05. |
| `R27-29` | `module-system` | `R27-28C` | Delete identity registry/freeze authority, session ledgers, and the session-owned handleful graph root. | Identity and session zero-reference gates | Complete; the identity registry/freeze implementation, dump/resolver bridges, registry-backed semantic-context fingerprint overload, factory claim state, and every production or unit-test consumer are deleted. Canonical identity interners and explicit immutable snapshots supply all retained identity and source paths. Production and unit-test zero-reference gates, sanitizer frontend build, 13 focused identity/Binder/Checker/type/driver tests, RFC validation, and diff check passed on 2026-08-06. |
| `R27-30` | `binder-checker` | `R27-28C` | Complete: deleted the frozen Binder inventory, production batch binding, detached clone oracles, non-owning bound-module input, and their dedicated build and test wiring. Retained materialized export-surface storage, parsed-module graph input, graph revision, and source-root failure projection now have independent interfaces. | Binder and Checker zero-reference gates | Complete; Binder batch API and deleted-path CMake zero-reference gates passed. Sanitizer frontend build plus `session-compiler-session-test`, `materialized-module-skeleton-test`, and `query-module-graph-module-graph-query-input-test` passed on 2026-08-06. |
| `R27-31` | `ir-backend` | `R27-27C` | Delete Binder-to-IR failure conversion and non-owning checked/HIR/MIR bound-module references. | IR zero-reference gates | Complete; checked, HIR, and MIR consumers retain `CheckerBoundModuleView` values, the deleted non-owning accessor and Binder-to-IR failure conversion have no production references, and sanitizer `hir-module-test`, `built-mir-test`, and `ir-diagnostic-adapter-test` passed on 2026-08-06. |
| `R27-31A` | `runtime-memory` | `R27-27D` | Delete non-owning ownership-overlay bound-module references. | Ownership zero-reference gate | Complete; ownership overlay retains the Built MIR bound-module lease by value, the non-owning overlay reference is absent, and sanitizer `overlay-ownership-event-overlay-test` passed on 2026-08-06. |
| `R27-31B` | `binder-checker` | `R27-30` | Synchronize Binder and Checker build source lists. | Focused build | Complete; Binder and Checker CMake source lists contain only present implementation files, and sanitizer `binder` and `checker` targets built on 2026-08-06. |
| `R27-31C` | `module-system` | `R27-29` | Synchronize identity, query, graph, and session build source lists. | Focused build | Complete; identity, query, and driver source lists contain no deleted identity-registry component and every listed implementation file exists. The sanitizer identity/query/driver/frontend target build passed on 2026-08-06. |
| `R27-31D` | `ir-backend` | `R27-31` | Synchronize compiler, checked-module, HIR, MIR, and IR build wiring. | Focused build | Complete; CMake contains no deleted checked-module input, bound-module input, or Binder-failure source entry, and sanitizer `frontend`, HIR, MIR, and IR diagnostic targets built on 2026-08-06. |
| `R27-31E` | `runtime-memory` | `R27-31A` | Synchronize ownership-overlay build wiring. | Focused build | Complete; ownership CMake wiring contains no deleted source entry, and sanitizer `overlay-ownership-event-overlay-test` built and passed on 2026-08-06. |
| `R27-32` | `verification` | RFC 0029 `R29-12AB`; RFC 0029 `R29-12D`; `R27-14A` through `R27-31E`; RFC 0029 `R29-15` | Implement the English-only and specification-alignment gates under their synchronized routing ownership; add the exact IR diagnostic regression and unique installed-consumer fixture; then run the complete sanitizer, unit, lit, codegen, architecture, format, versioning, frozen-base coverage, English-only, install-consumer, spec-alignment, and Release compare matrix. | RFC 0027, RFC 0028, and RFC 0029 Test Plans | Complete; the sanitizer build, all 257 default CTest entries, ownership coverage, static architecture and governance gates, specification alignment, and fixed core-alignment report verification passed. The Release benchmark catalog now selects only a SHA-256 identity derived from build, compiler, corpus, machine, and worker metadata; independently reviewed Darwin and Linux entries coexist without normalization. The Linux Release comparison passed on 2026-08-07. |
| `R27-33A` | `spec-audit` | `R27-32` | Update current-state compiler design documentation from the landed production path and publish `docs/reports/zom-core-library-spec-alignment.md` with the report-producing core alignment command. | Spec and design audit plus fixed report publication | Complete; the current-state query-runtime evidence map names the exact-metadata Release baseline catalog, and the report-producing core-alignment command plus byte-for-byte report verification passed on 2026-08-07. |
| `R27-32A` | `verification` | `R27-33A` | Recompute and byte-compare the fixed report without writing it, then rerun English-only, RFC, format, versioning, and diff gates after every spec-audit edit. | Final write-free RFC 0027 Test Plan subset | Complete; fixed-report byte comparison, English-only, RFC, formatting, internal-versioning, and diff gates passed after the cross-platform evidence update on 2026-08-07. |
| `R27-33` | `rfc` | `R27-32A` | Audit all implementation, report, and final verification evidence, then perform the only synchronized RFC implementation-state transitions. | `python3 scripts/check-rfc.py` | Complete; the audit confirms every R27 tracker task through `R27-32A`, transitions RFC 0027 to IMPLEMENTING, and records that independent incomplete RFC 0025 work prevents a synchronized LANDED claim. |
