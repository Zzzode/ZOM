...
| `R27-26` | `binder-checker` | `R27-25` | Complete: Checker fact builders consume `CheckerBoundModuleView`, while `CheckerIdentityAuthority` retains one dependency-ordered verified view per materialized graph module and resolves the complete eight-domain identity authority. | Focused Checker tests | Complete; focused authority coverage plus the 39-test sanitizer `compiler-session-package-test` suite passed on 2026-08-05. |
| `R27-26A` | `module-system` | `R27-26` | Complete: module-interface publication owns and retains an `OwnershipAdmittedBoundModule`. | Focused interface tests | Complete; module-interface teardown retention coverage passed on 2026-08-05. |
| `R27-27A` | `ir-backend` | `R27-26` | Complete: verified checked modules retain an `OwnershipAdmittedBoundModule`. | Checked-module lineage tests | Complete; package pipeline lineage coverage passed on 2026-08-05. |
| `R27-27B` | `ir-backend` | `R27-27A` | Complete: verified HIR retains an `OwnershipAdmittedBoundModule`. | HIR lineage tests | Complete; scalar-return HIR lineage coverage passed on 2026-08-05. |
| `R27-27C` | `ir-backend` | `R27-27B` | Complete: Built MIR retains an `OwnershipAdmittedBoundModule`. | MIR lineage tests | Complete; scalar-return MIR lineage coverage passed on 2026-08-05. |
| `R27-27D` | `runtime-memory` | `R27-27C` | Complete: ownership overlays retain an `OwnershipAdmittedBoundModule` and session teardown releases them first. | Ownership lineage tests | Complete; teardown order and package pipeline coverage passed on 2026-08-05. |
...
| `R27-30` | `binder-checker` | `R27-28C` | Complete: deleted the frozen Binder inventory, production batch binding, detached clone oracles, non-owning bound-module input, and their dedicated build and test wiring. Retained materialized export-surface storage, parsed-module graph input, graph revision, and source-root failure projection now have independent interfaces. | Binder and Checker zero-reference gates | Complete; Binder batch API and deleted-path CMake zero-reference gates passed. Sanitizer frontend build plus `compiler-session-test`, `materialized-module-skeleton-test`, and `module-graph-query-input-test` passed on 2026-08-06. |
...
| `R27-31A` | `runtime-memory` | `R27-27D` | Delete non-owning ownership-overlay bound-module references. | Ownership zero-reference gate | Complete; ownership overlay retains the Built MIR bound-module lease by value, the non-owning overlay reference is absent, and sanitizer `ownership-event-overlay-test` passed on 2026-08-06. |
...
| `R27-31E` | `runtime-memory` | `R27-31A` | Synchronize ownership-overlay build wiring. | Focused build | Complete; ownership CMake wiring contains no deleted source entry, and sanitizer `ownership-event-overlay-test` built and passed on 2026-08-06. |
