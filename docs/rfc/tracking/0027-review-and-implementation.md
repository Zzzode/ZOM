...
| `R27-21` | `binder-checker` | `R27-20` | Implement contextual `BindOwnerBody` and independent traversal/verifier. | Body, capture, control, and source-failure tests | In progress; detached syntax paths, ancestry, scope classification, and canonical scope/node-scope projection are rebuilt and independently verified with nested-root, empty-body, and inherited-root coverage. Definition owners now expose only their selected executable root with independently rebased provenance. Provider and verifier independently select callable parameter scopes and field or constant declaration scopes from the exact owning skeleton, and reject foreign module roots or mismatched definition roots, including empty syntax. The canonical detached-node reader now maps schema node fields to checked immediate-child ordinals; the provider consumes it while the verifier retains an independent payload decode. Local binding projection now covers simple and destructuring `let` patterns, including inherited owner-root scopes. Lambda projection now publishes one stable closure and one empty free-variable fact before lookup reconstruction. Label projection now records block and loop targets, and control projection validates explicit loop-label transfers before lookup reconstruction. The retained semantic descriptor now has canonical key and result codecs, adapter registration, and sealed-session publication coverage for every active body. |
| `R27-22` | `binder-checker` | `R27-21` | Implement the deterministic five-domain module allocation plan. | Overflow and reversed-demand tests | In progress; the pure planner independently validates canonical owner coverage and computes checked dense scope, owner-local, anonymous-owner, and label ranges from a published skeleton and body facts. Its retained contextual semantic descriptor now demands the exact skeleton and every canonical `BindOwnerBody` result, forwards typed upstream failures, and verifies the independently rebuilt plan with sealed-session codec coverage. |
