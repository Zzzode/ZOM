...
| Publish the verified checked-module handoff | Complete for the current checked-module surface through RFC 0027 `R27-27A` | The final-snapshot `OwnershipAdmittedBoundModule` is retained and independently verified through context roots, revision, fingerprint, module, graph and skeleton witnesses, owner coverage, and expanded handles |
| Implement semantic HIR and verifier | Complete for the current semantic HIR surface through RFC 0027 `R27-27B` | HIR retains the checked-module admitted bound-module capability and verifies field projection, imported-interface lineage, deterministic revision, mutation coverage, and dumps |
| Implement Built MIR and RFC 0013 ownership integration | Complete for the current Built MIR and ownership-event-overlay handoff through RFC 0027 `R27-27C` and `R27-27D` | Built MIR and ownership overlays retain the admitted bound-module capability with verified revision, current exit coverage, destruction order, and permutation evidence; complete ownership proof publication remains RFC 0007 work |
