...
yet lower unsafe blocks. Nested component discovery,
direct deinitializers, structural cleanup, dynamic dispatch, and cast routing
are also absent. The Built MIR and ownership event overlay row therefore
...
- `DropElaborator` and `DropElaboratedMir` form the sole committed successor
  of `OwnershipCheckedMir`. The elaborator consumes the checked wrapper,
  rechecks every revision, lease, and identity, validates that every pending
  drop obligation has a complete discharge path through the linear CFG, and
  publishes one wrapper with a recorded `DropDischargeRecord` inventory. It
  is implemented, unit-tested, and wired into the session pipeline:
  immediately after `finalizeOwnership` succeeds, the session calls
  `elaborateDrops` on the consumed `OwnershipCheckedMir`, extracts the
  checked MIR via `takeCheckedMir()`, and commits both
  `ownershipCheckedMirModules` and `dropElaboratedMirModules` atomically. A
  rejected finalization or elaboration destroys its consumed local input and
  publishes no predecessor or partial successor.
