...
with byte-mutation coverage. Production lowering emits no boundary statements
today. Nested component discovery,
...
- `DropElaborator` and `DropElaboratedMir` form the sole committed successor
  ...
  is implemented and unit-tested but not yet wired into the session pipeline.
