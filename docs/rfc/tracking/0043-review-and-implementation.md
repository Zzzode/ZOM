...
| Canonical link-plan construction and verification | Landed 2026-08-28; contract refined and landed 2026-08-29 (`320a2a01`) | Initial landed shape (`compiler/ir/link-plan-codec.{h,cc}`): the `ToolchainClosure`/input/argument records, the domain-separated length-framed `LinkPlanId` codec, and the independent `LinkPlanVerifier` enforcing the six invariants; `link-plan-codec-oracle-test` froze a 503-byte minimal-plan oracle (`LinkPlanId` `287f421b...52a405`, superseded) and a fail-closed mutation matrix (no linker). The failure algebra was extended in code (`LinkPlanConstruction`/`LinkerInvocation`/`ExecutablePublication` phases + `InvokeLinker` op). Current landed shape (`320a2a01`, 2026-08-29 refinement): the generic argument surface is removed - no `LinkerArgumentRecord`, no argument sequence in the codec preimage, no argument invariant - because it allowed a snapshot bypass and has no production producer; the driver derives its canonical argument vector from the closed structural fields. The oracle was regenerated from the live encoder: 469 bytes, `LinkPlanId` `8e9a5cf7...5d60ca74`. |
...
| Executable inspection, manifest, and recoverable publication | D1 implementation candidate pending adversarial review; D5 Pending | Manifest-last journal transaction and recovery matrix review; ELF/Mach-O verifier and consuming `linkAndPublish` |
