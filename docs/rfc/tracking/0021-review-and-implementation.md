...
| LIR identity, carrier, layout, ABI, and revision foundation | Landed 2026-08-27 (`fabf520d`) | Closed stores, independent oracle, exact mutation matrix - DELIVERED: compiler/lir/ (five branded identities; immutable carrier/layout/FnAbi/runtime-symbol/source-location records + interning stores; LirAlgebraCodec whose live encoder reproduces the documented 56-byte empty-registry oracle + SHA-256 03106c34...). Oracle + mutation + fail-closed tests pass; no LLVM linkage. |
...
- No production code includes or links LLVM today: a repository grep for
  ...
  `target_link_libraries` across `compiler/`, `libraries/`, and `cmake/`
