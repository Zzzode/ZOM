...
`compiler/identity/key/compilation-unit-key.h`, and
`compiler/identity/key/compilation-unit-key.cc`. The dedicated
...
`tests/fixtures/package-oracles/package-generated-oracles.json`.
...
- The Implementation Tracker cites 37 paths that no longer exist, spread across
  14 rows: `R25-02BA`, `R25-02B`, `R25-02C`, `R25-02P`, `R25-03`, `R25-03T`,
  `R25-03CT`, `R25-05G`, `R25-07`, `R25-07T`, `R25-08T`, `R25-09C`, `R25-09D`,
  and `R25-12A`. A separate set of 27 citations named files that only moved
  during the source-grouping refactors; those were corrected in place on
  2026-09-07 and are not part of this count.

  Most of the 37 did exist and were deleted deliberately. The identity registry
  and freeze authority, its dump and resolver bridges, and their tests were
  removed by RFC 0027 stage `R27-29` on 2026-08-06; the diagnostics artifacts
  were removed by RFC 0047 on 2026-09-06; and `tests/unittests/tools/{ide,lsp}/`
  moved wholesale to `compiler/ide/` and `compiler/lsp/`. A path-scoped history
  query cannot see this, because commit `f89d74b6` moved the whole tree out from
  under `products/zomlang/`; search by symbol with `git log -S` instead.

  Seven of the 14 rows are marked `Complete`. Their completion claims are not
  withdrawn: the work was real when it was recorded, and a later RFC removing
  the artifact does not retroactively make the row false. What those rows can no
  longer do is serve as reproducible evidence, because the files they name are
  gone. The other seven are `Blocked` or `In progress` and plan against absent
  artifacts, so they are unexecutable as written.

  Per the 2026-09-07 audit, this tracker needs one rewrite pass owned by RFC
  0025 rather than line fixes, and that pass should state the `R25-03` versus
  `R27-29` relationship explicitly so neither row reads as contradicting the
  other. Recorded here rather than fixed under a diagnostics authorization.
