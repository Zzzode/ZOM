...
| Canonical stable schema | `compiler/binder/stable-binding-schema.def` | `binder-checker` |
| Input and capability ownership | `compiler/driver/**`; `compiler/query/**` | `module-system` |
| Capability result and lease lifetime | `compiler/query/query-types.{h,cc}` | `runtime-memory` |
| Diagnostic outer sums and mappings | `compiler/diagnostics/**`; `compiler/checker/checker-source-diagnostics.def` | `error-system` |
| Schema gate, mutation self-test, and native test ownership | `scripts/check-stable-binding-schema.py`; `tests/**` | `verification` |
...
- `python3 scripts/check-english-only.py --check --base-file
  tests/coverage/implementation-series-base.txt` passes.
