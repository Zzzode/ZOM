...
source digests before use. CI additionally exercises forced misses and
intentional nondeterminism fixtures.
...
|---|---:|---:|---|
| `read` | 0 | 63 | Descriptor 3 or one regular input descriptor returned by the runtime |
| `write` | 1 | 64 | Descriptor 4 or one regular output descriptor returned by the runtime |
| `close` | 3 | 57 | Descriptors 3-7 or a runtime-opened regular file; double close is rejected by the runtime state machine |
| `fstat` | 5 | 80 | One runtime-opened regular input or output descriptor only |
...
descriptors 3-7 and one runtime-opened file can coexist and the response channel
...
environment value, and 4 MiB cumulative exported environment bytes.
...
`products/zomlang/compiler/driver/package/vendor/<dependency>/**`. The driver
...
source inventories above. `products/zomlang/tests/tools/check-vendored-dependencies.py` owns a
